...
{
	$cadena = mysqli_query(Conectarse(), "SELECT pe_hr_ideal FROM preparacion_etapas WHERE pe_id = '$intEtapa' ") or die(mysqli_error(Conectarse()) . "Error: en consultar la etapa");
...
{
	$cadena = mysqli_query(Conectarse(), "SELECT pe_hr_maxima FROM preparacion_etapas WHERE pe_id = '$intEtapa' ") or die(mysqli_error(Conectarse()) . "Error: en consultar la etapa");
	$registros = mysqli_fetch_assoc($cadena);
...
{
	$cadena = mysqli_query(Conectarse(), "SELECT pe_inicio FROM preparacion_etapas WHERE pe_id = '$intEtapa' ") or die(mysqli_error(Conectarse()) . "Error: en consultar la etapa");
	$registros = mysqli_fetch_assoc($cadena);
...
{
	$cadena = mysqli_query(Conectarse(), "SELECT pe_fin FROM preparacion_etapas WHERE pe_id = '$intEtapa' ") or die(mysqli_error(Conectarse()) . "Error: en consultar la etapa");
	$registros = mysqli_fetch_assoc($cadena);
...
{
	$cadena = mysqli_query(Conectarse(), "SELECT usu_usuario FROM usuarios WHERE usu_id = '$intUsu' ") or die(mysqli_error(Conectarse()) . "Error: en consultar el usuario");
	$registros = mysqli_fetch_assoc($cadena);
	if (isset($registros['usu_usuario'])) {
...
{
	$cadena = mysqli_query(Conectarse(), "SELECT pe_fin FROM preparacion_etapas WHERE pe_id = '$intEtapa' and pe_tipo = '$intTipo' ") or die(mysqli_error(Conectarse()) . "Error: en consultar la etapa");
	$registros = mysqli_fetch_assoc($cadena);
...
function fnc_valida_etapa($intPro, $intEta)
{
	...
	if (empty($registros['prol_id'])) // and $registros2['prol_id'] != ''
	{
		/*if($intEta == 1)
		{*/
		return "Si";
...
//formato en horas - almacenamiento
function fnc_horas_insertar($strFeIni, $strFeFin, $strHrIni, $strHrFin)
{
...
//formato en hora - visualización
function fnc_horas($strFeIni, $strFeFin, $strHrIni, $strHrFin)
{
	$date1 = new DateTime($strFeIni . " " . $strHrIni);
	$date2 = new DateTime($strFeFin . " " . $strHrFin);

	/* echo "fecha y hora inicio ".$strFeIni.' '.$strHrIni."<br>";
	echo "fecha y hora fin ".$strFeFin.' '.$strHrFin."<br><br>"; */
