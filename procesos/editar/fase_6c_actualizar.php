...
mysqli_query($cnx, "UPDATE procesos_fase_6b_g SET tpa_id = '$cbxTipAg' WHERE pro_id = '$hdd_pro_id' AND pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx)." Error al actualizar 2");
...
for($i = 1; $i <= 10; $i++)
{
	...
	if($cbxTipAgd != '' or $txtTemp != '' or $txtHraIni != '' or $txtHraFin != '' or $txtHraFin != ''  or $txtHraIniMov != '' or $txtHraFinMov != '' or $txtPh != '' or $txtCe != '')
	{
		if($cbxTipAgd != ''){$str_update .= ",tpa_id = '$cbxTipAgd'";}
		if($txtTemp != ''){$str_update .= ", pfd6_temp = '$txtTemp'";}
		if($txtHraIni != ''){$str_update .= ", pfd6_hr_ini = '$txtHraIni'";}
		if($txtHraFin != ''){$str_update .= ", pfd6_hr_fin = '$txtHraFin'";}
		if($txtHraIniMov != ''){$str_update .= ", pfd6_hr_ini_mov = '$txtHraIniMov'";}
		if($txtHraFinMov != ''){$str_update .= ", pfd6_hr_fin_mov = '$txtHraFinMov'";}
		if($txtPh != ''){$str_update .= ", pfd6_ph = '$txtPh'";}
		if($txtCe != ''){$str_update .= ", pfd6_ce = '$txtCe'";}
		if($txtagua_a != ''){$str_update .= ",taa_id = '$txtagua_a'";}
		if($txtObs != ''){$str_update .= ", pfd6_observaciones = '$txtObs' ";}

		/*mysqli_query($cnx, "UPDATE procesos_fase_6b_d SET pfd6_temp = '$txtTemp', pfd6_hr_ini = '$txtHraIni', pfd6_hr_fin = '$txtHraFin', pfd6_hr_ini_mov = '$txtHraIniMov', pfd6_hr_fin_mov = '$txtHraFinMov', pfd6_ph = '$txtPh', pfd6_ce = '$txtCe',taa_id = '$txtagua_a', pfd6_observaciones = '$txtObs' WHERE pfg6_id = '$hdd_pfg' AND pfd6_ren = '$txtRen' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);*/
