...
for($i = 1; $i <= 6; $i++)
{
	...
	{ */
		...
		mysqli_query($cnx, "UPDATE procesos_fase_7b_d2 SET pfd7_ini_mov = '$txtIniMovD', pfd7_ini_reposo = '$txtIniRepD', pfd7_ph = '$txtPhD', pfd7_norm = '$txtNormD', pfd7_temp = '$txtTempD' WHERE pfg7_id = '$hdd_pfg' AND pfd7_ren = '$txtRen2' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);
	//}
}
...
{
	mysqli_query($cnx, "UPDATE procesos_fase_7b_g SET pfg7_hr_ini_co = '$txtHrIniC', pfg7_agua_ph = '$txtPhR1', pfg7_cocido_ph = '$txtPhR2', usu_sup = '".$_SESSION['idUsu']."' WHERE pfg7_id = '$hdd_pfg' ") or die(mysqli_error($cnx)." Error al actualizar 7-B");
}

if($txtCeR1 != '' and $txtCeR2 != '')
{
	mysqli_query($cnx, "UPDATE procesos_fase_7b_g SET pfg7_agua_ce = '$txtCeR1', pfg7_cocido_ce = '$txtCeR2'  WHERE pfg7_id = '$hdd_pfg' ") or die(mysqli_error($cnx)." Error al actualizar 7-C");
}
