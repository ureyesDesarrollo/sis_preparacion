...
else
{
	for($i = 1; $i <= 10; $i++)
	{
		...
		/*if($txtMov != '' and $txtIniLlen != '' and $txtFinLlen != '' and $txtIniDren != '' and $txtFinDren != '' and $txtPh != '' and $txtCe != '' and $txtTemp != '' )//  or ( and $i <= 3)
		{ 
			mysqli_query($cnx, "INSERT INTO procesos_fase_8_d(pfg8_id, pfd8_ren, pfd8_mov, pfd8_ini_llenado, pfd8_fin_llenado, pfd8_ini_dren, pfd8_fin_dren, pfd8_ph, pfd8_ce, pfd8_temp, usu_id, pfd8_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen', '$txtMov', '$txtIniLlen', '$txtFinLlen', '$txtIniDren', '$txtFinDren', '$txtPh', '$txtCe', '$txtTemp', '".$_SESSION['idUsu']."', SYSDATE() ) ") or die(mysqli_error($cnx)." Error al insertar B".$i);
	...
	else
	{
		...
		else
		{
			...
			if($txtFeLib != '' and $txtHrLib != '' and $txtPhLibc1 != '' and $txtCeLibc1 != '' and $txtCue_sobc1 != '' and $txtpor_extc1 != '' and $txtPhLibc2 != '' and $txtCeLibc2 != '' and $txtCue_sobc2 != '' and $txtpor_extc2 != '')
			{
				...
				//}
				//mysqli_query($cnx, "UPDATE preparacion_paletos SET le_id = '2' WHERE pp_id = '$hdd_pal' ") or die(mysqli_error($cnx)." Error al actualizar z"); //Libera el Paleto
