...
if($txtFeIni != '')
{
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_1_g(pro_id, pe_id, pfg1_ph_agua, pfg1_ce_agua) VALUES('$hdd_pro_id',  '$hdd_pe_id', '$txtAgIni', '$txtCeIni' ) ") or die(mysqli_error($cnx)." Error al insertar 2");
	
	$respuesta = array('mensaje' => "Fase 1b agregada");
}
else
{
	for($i = 1; $i <= 10; $i++)
	...
	if($strMsj == '')
	{
		...
		else
		{
			if($txtHrTotales != '')
			{
				...
				$strMsj = "Fase 1b parametros capturados";
			}
			else
			{
				$strMsj = "Esta vacio";
