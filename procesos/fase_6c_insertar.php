...
if($txtFeIni != '')
{
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_6b_g(pro_id, pe_id, tpa_id, usu_id) VALUES('$hdd_pro_id', '$hdd_pe_id', '$cbxTipAg', '".$_SESSION['idUsu']."' ) ") or die(mysqli_error($cnx)." Error al insertar 2");
	
	$respuesta = array('mensaje' => "Fase 6c agregada");
}
else
{
	for($i = 1; $i <= 10; $i++)
	{
		...
		{//$cbxTipAg != '' and 
			mysqli_query($cnx, "INSERT INTO procesos_fase_6b_d(pfg6_id, pfd6_ren, tpa_id, pfd6_temp, pfd6_hr_ini, pfd6_hr_fin, pfd6_hr_ini_mov, pfd6_hr_fin_mov, pfd6_ph, pfd6_ce,  usu_id, pfd6_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen', '0', '$txtTemp', '$txtHraIni', '$txtHraFin', '$txtHraIniMov', '$txtHraFinMov', '$txtPh', '$txtCe', '".$_SESSION['idUsu']."', SYSDATE() ) ") or die(mysqli_error($cnx)." Error al insertar 1");
			
			$strMsj = "Se agrego el renglon ".$i;
		}*/
		if($hddRen == '' and $txtTemp != '' and $cbxTipAg != '')
		{
			mysqli_query($cnx, "INSERT INTO procesos_fase_6b_d(pfg6_id, pfd6_ren, tpa_id,pfd6_temp, usu_id, pfd6_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen','$cbxTipAg' ,'$txtTemp', '".$_SESSION['idUsu']."', SYSDATE() ) ") or die(mysqli_error($cnx)." Error al insertar 1");
			$strMsj = "Se agrego el renglon ".$i;
		}
		else
		{ 
		...
		else
		{
			if ($reg_usu['up_id'] != '3') {
				if($txtHrTotales != '')
				{
					...
					$strMsj = "Fase 6c parametros capturados";
				}
				else
				{
					$strMsj = "Esta vacio";
