...
$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '20'") or die(mysql_error()."Error: en consultar procesos auxiliar");
$regProAux = mysqli_fetch_assoc($sqlProAux);

$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion_b WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '20'") or die(mysql_error()."Error: en consultar procesos de liberacion");
...
<div class="divProcesos">
  <table width="100%" style="margin:20px 0px 20px 0px">
    <tr>
      <td colspan="2"><table width="90%" border="1" style="background: #FCEFF2;font-size: 12px;width: 99%">
        <tr>
          <td height="45" colspan="14">LAVADOS FINALES. Este proceso se utilizará: Agua recuperada de los ultimos lavados finales de otro paleto y/o agua limpia
            <div></div></td>
          </tr>
      </table></td>
      </tr>
      <tr>
        <td colspan="2"><table width="99%" border="1">
          <tr>
      ...
      <tr>
        <td colspan="2">
          <table width="1119" border="1" style="margin-top: 10px">
            <tr style="font-weight: bold;background: #e6e6e6">
              ...
              <td width="10%" style="font-size: small; text-align: center;">Agua a</td>
              <td style="font-size: small; text-align: center;">Observaciones</td>
              <td style="font-size: small; text-align: center;">Capturo</td>
              <td rowspan="2" >Para hacer los lavados finales deben tardar de (<span style="background: #e6e6e6"><?php echo fnc_hora_de(20) ?> a <?php echo fnc_hora_a(20) ?></span> horas)</td>
  ...
  $agua_a = mysqli_query($cnx, "SELECT * FROM tipos_agua_a WHERE taa_id = '$regfd[taa_id]'") or die(mysql_error()."Error: en consultar el tipo de agua a");
        ...
        <tr>
          <td width="54%" valign="top"><table border="1" width="679">
          ...
          <td width="46%" rowspan="3" valign="top"><table width="431" border="1">
            <tr>
        ...
        <tr>
          <td valign="top"><table border="1" width="679">
            <tr style="font-weight: bold;">
              <td width="210" style="background: #e6e6e6">Horas totales de todo el proceso</td>
              <td width="96" style="background: #e6e6e6">Usuario</td>
              <td width="124" rowspan="2" style="background: #e6e6e6">(<?php echo fnc_hora_de(20) ?> a <?php echo fnc_hora_a(20) ?> horas)</td>
            </tr>
            <tr>
              <td><?php echo $regfg['pfg8_hr_totales']?></td>
              <td><?php echo fnc_nom_usu($usu_aux)?></td>
            ...
            <tr style="font-weight: bold;">
              ...
              <td width="232" rowspan="2" style="background: #e6e6e6">(<?php echo fnc_hora_de(20) ?> a <?php echo fnc_hora_a(20) ?> horas)</td>
            </tr>
            <tr>
		...
		$cad = mysqli_query($cnx, "SELECT pl_id, pro_estatus FROM procesos WHERE pro_id = '$idx_pro' ") or die(mysql_error()."Error: en consultar");
