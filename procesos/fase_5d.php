...
{	
	$("#formFase5d").submit(function(){

		var formData = $(this).serialize();
		$.ajax({
			url: "fase_5c_insertar.php",
			type: 'POST',
			data: formData,
			success: function(result) {

				data = JSON.parse(result);
				alertas("#alerta-errorFase5dOpe", 'Listo!', data["mensaje"], 1, true, 5000);
				$('#formFase5d').each (function(){this.reset();}); 
	...
	//agregado 16-10-21
	function quimicos_5d(proceso, etapa){
		var datos={
			"pro_id": proceso,
			"pe_id": etapa
		}
...
<form id="formFase5d" name="formFase5d">
	...
	<div class="row" style="margin-bottom: 20px">
		...
		<div class="col-md-1 tiempos">
			<input type="text" class="form-control" id="txtTermina" name="txtTermina" onKeyPress="return isNumberKeyFloat(event, this);" onKeyPress="return isNumberKeyFloat(event, this);" placeholder="Termina" value="<?php if($reg_fa['pfg5_termina'] == ''){echo ""; $str_prop = '';}else{echo $reg_fa['pfg5_termina']; $str_prop = 'disabled';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3; ?> required>
		</div>
		<label class="col-md-1" >Temp</label>
		<div class="col-md-1 tiempos">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			for($i = 1; $i <=8; $i++){ 
				...
				<tr>
					...
					<td><input type="text" class="form-control" id="<?php echo "txtPhBF".$i ?>" name="<?php echo "txtPhBF".$i ?>" size="5" value="<?php if($reg_fad['pfd5_ph'] == ''){echo ""; $str_prop = '';}else{echo $reg_fad['pfd5_ph']; $str_prop = 'disabled';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3; echo " ".$strProp4; ?> placeholder="pH B"></td>
...
<!--barra botones-->
	<div class="row footerdivProcesos" style="margin-bottom: 10px">
		...
		<div class="form-group col-md-3">
			<div class="alert alert-info hide" id="alerta-errorFase5dOpe" style="height: 40px;width: 270px;text-align: left;z-index: 10;font-size: 10px;">
	...
	<?php if($_SESSION['privilegio'] == 4){	?>
		...
		<?php }?>
				...
				<?php 
					...
					<button type="button" class="btn btn-info"  onClick="javascript:quimicos_5d(<?php echo $reg_pro['pro_id'] ?>, <?php echo $reg_et['pe_id'] ?>);"><img src="../iconos/matraz.png" alt="">Químicos</button>
