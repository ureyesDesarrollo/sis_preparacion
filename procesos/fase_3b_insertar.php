...
{ //echo $txtTempA;
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_3b_g(pro_id, pfg3_temp_ag, pfg3_lts, pfg3_ph, pfg3_norm, pfg3_temp, usu_id) VALUES('$hdd_pro_id', '$txtTempA', '$txtSosa', '$txtPh', '$txtNorm', '$txtTemp', '".$_SESSION['idUsu']."' ) ") or die(mysqli_error($cnx)." Error al insertar 2");
...
else
{
	for($i = 1; $i <= 44; $i++)
	{
		...
		if($txtFecha != '' and $txtHora != '' and $txtTemp != '' and $txtNorm != '' and $txtMovimiento != '' and $txtReposo != '' and $txtSosa != '')
		{
			...
			/*fnc_alertas($hdd_pe_id, 'N', $hdd_pro_id, $txtNorm, $_SESSION['idUsu'], $lavador, $paleto, 'R');
			fnc_alertas_v2($hdd_pe_id, 'Temp', $hdd_pro_id, $txtTemp, $_SESSION['idUsu'], $lavador, $paleto, 'R');*/
...
/*	for($i = 7; $i <= 44; $i++)
	{
		...
		if($txtHora2 != '' and $txtPh2 != '' and $txtTemp2 != '' and $txtMinMov != '' and $txtReposo != '')
		{
			mysqli_query($cnx, "INSERT INTO procesos_fase_2b_d2(pfg2_id, pfd22_ren, pfd22_hr, pfd22_ph, pfd22_temp, pfd22_reposo, pfd22_min, usu_id, pfd22_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen2', '$txtHora2', '$txtPh2', '$txtTemp2', '$txtMinMov', '$txtReposo', '".$_SESSION['idUsu']."', SYSDATE() ) ") or die(mysqli_error($cnx)." Error al insertar B".$i);
			$strMsj = "Se agrego el renglon ".$i;
		}
	}*/
