...
$cad_pro = mysqli_query($cnx, "SELECT p.*, x.pp_id   
							   FROM procesos as p
							   ...
							   WHERE x.pp_id = '$id_p' AND x.prop_estatus = 1");
...
<input name="hdd_pro_id" type="hidden" value="<?php echo $reg_pro['pro_id'] ?>" id="hdd_pro_id"/>
	<div class="form-row">
		...
		<div class="form-group col-md-3" >
			...
			<select id="cbxLavador" class="form-control" style="width: 250px" name="cbxLavador" required disabled="disabled">
				...
				<?php 
				$cadena =  mysqli_query($cnx,"SELECT * from preparacion_paletos");
				$registros =  mysqli_fetch_array($cadena);

				do
				{
					?><option value="<?php echo $registros['pp_id'] ?>" <?php if($registros['pp_id'] == $reg_pro['pp_id'] ) {?> selected="selected" <?php }?>><?php echo $registros['pp_descripcion'] ?></option><?php
				}while($registros =  mysqli_fetch_array($cadena));
	...
	<div class="form-row">
		...
		<div class="form-group col-md-1" style="width: 130px">
			...
			<?php /*for($i = 1; $i <= 5; $i++){ ?>
				<input class="form-control" type="text" class="form-control" id="<?php echo "txtFecha".$i ?>" style="width: 100px" name="<?php echo "txtFecha".$i ?>"  readonly="true" size="10">
			<?php }*/
			$sqlProMat = mysqli_query($cnx, "SELECT m.mat_nombre, pm.pma_kg, pm.pma_fe_entrada 
            FROM materiales as m 
            INNER JOIN procesos_materiales as pm on(m.mat_id=pm.mat_id) 
		...
		<div class="form-group col-md-1" style="width: 130px">
			<label for="inputPassword4">F. entrada maq</label>
			<?php 
          ...
          do{?>
			<input  class="form-control" name="txtFecha_ent_maquila" type="text" id="txtFecha_ent_maquila" value="<?php echo $regProMat['pma_fe_entrada_maquila'] ?>" readonly="" style="width:90px;" disabled="disabled"/>
			<?php }while($regProMat= mysqli_fetch_assoc($sqlProMat));?>
		</div>
	</div>

	<?php 
	include "tabla_1b.php";
	include "tabla_2.php";
	?>
	<div class="row">
		<div class="col-md-12">
			...
			<div class="form-group col-md-2">
				<label for="inputPassword4">Fecha  carga paleto:</label>
