...
try {
    ...
    $cantidad = floatval($datos_detalle_embarque[0]['cantidad_solicitada']) * floatval($datos_detalle_embarque[0]['pres_kg']);
    ...
    $listado_revolutura = "SELECT r.*,ROUND(r.rev_bloom) AS rev_bloom,ROUND(r.rev_viscosidad) AS rev_viscosidad
    ...
    $fecha_caducidad = clone $fecha_elaboracion;
    $fecha_caducidad->modify('+5 years');
    ...
    $calidad = obtenerBloomPorCalidad($revoltura['cal_id']);
} catch (Exception $e) {
