...
try {
    ...
    ^FO100,90^A0N,70,70^FDProceso: {$datos_tarimas['pro_id']}{$pro_id_2}^FS
    ^FO100,160^A0N,70,70^FDTarima: {$datos_tarimas['tar_folio']}^FS
    ^FO100,230^A0N,70,70^FD{$fecha_formateada}^FS

    /* Solo imprime si $fino tiene un valor */
    ^IF{$fino}^
        ^FO100,300^A0N,70,70^FD{$fino}^FS
    ^ENDIF

    /* Primer QR (tamaño fijo y con espacio definido) */
    ^FO250,380^BQN,2,7^FDQA,{$url}^FS  

    /* Línea horizontal de separación */
    ^FO50,740^GB700,3,3^FS  

    /* Segundo bloque de información */
    ^FO100,810^A0N,70,70^FDProceso: {$datos_tarimas['pro_id']}{$pro_id_2}^FS
    ^FO100,880^A0N,70,70^FDTarima: {$datos_tarimas['tar_folio']}^FS
    ^FO100,950^A0N,70,70^FD{$fecha_formateada}^FS

    /* Solo imprime si $fino tiene un valor */
    ^IF{$fino}^
        ^FO100,1020^A0N,70,70^FD{$fino}^FS
    ^ENDIF

    /* Segundo QR (tamaño fijo y con espacio definido) */
    ^FO250,1090^BQN,2,7^FDQA,{$url}^FS  
