...
/*Actualizado: Marzo-2026 — Se agrega token de idempotencia para evitar duplicados por retransmisión de red */
...
try {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $tar_fino       = isset($_POST['chk_tamiz_nova']) ? 'T' : $tar_fino;

        // ─── TOKEN DE IDEMPOTENCIA ───────────────────────────────────────────
        // Recibir el token enviado desde el frontend
        $tar_token = mysqli_real_escape_string($cnx, $_POST['tar_token'] ?? '');

        // Si el token está vacío, rechazar la petición
        if (empty($tar_token)) {
            echo json_encode(["error" => "Token de seguridad inválido. Por favor recarga la página."]);
            exit();
        }

        // Verificar si este token ya fue procesado (petición duplicada por red)
        $check_token = mysqli_query($cnx, "SELECT tar_id FROM rev_tarimas WHERE tar_token = '$tar_token' LIMIT 1");
        if (mysqli_num_rows($check_token) > 0) {
            // Devolver éxito silencioso con el tar_id original para que el frontend
            // pueda continuar con la impresión del QR normalmente
            $existing = mysqli_fetch_assoc($check_token);
            echo json_encode([
                "success" => "Nuevo registro creado exitosamente. ",
                "tar_id"  => $existing['tar_id']
            ]);
            exit();
        }
        // ─────────────────────────────────────────────────────────────────────
        ...
        if ($tar_kilos > 0 && $tar_kilos <= 1000) {
            ...
            // Incluir tar_token en el INSERT
            $sql = "INSERT INTO rev_tarimas (pro_id, tar_folio, usu_id, tar_fino, tar_kilos, tar_token)
                    VALUES ('$pro_id','$tar_folio','$usu_id','$tar_fino','$tar_kilos','$tar_token')";
