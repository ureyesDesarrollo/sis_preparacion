...
try {
    ...
    $sql_update = "UPDATE orden_devolucion_detalle SET estado_lote = 'EN ALMACEN' WHERE odd_id = $odd_id";
    if (!mysqli_query($cnx, $sql_update)) {
        throw new Exception("Error al actualizar orden de devolución: " . mysqli_error($cnx));
    }

    $msg = "Lote recibido en almacen correctamente.";

    // Verificar si todos los lotes han sido recibidos en almacen
    ...
    if ($todos_recibidos) {
        ...
        $sql_update_orden = "UPDATE orden_devolucion SET od_estado = 'PROCESO', od_fecha_recibe = NOW() WHERE od_id = $od_id";
