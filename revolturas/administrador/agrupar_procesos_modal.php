    ...
    try {
        $listado_procesos_recorte = mysqli_query(
            ...
            "SELECT
                ...
                l.lote_folio,
                a.pro_id_pa,
                GROUP_CONCAT(DISTINCT m.mat_nombre ORDER BY m.mat_nombre SEPARATOR ', ') AS materiales,
                pt.pt_descripcion,
                pt.pt_id
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        procesos.forEach(function(pro) {
                            options += `<option value="${pro.pro_id}">${pro.pro_id} - ${pro.materiales}</option>`;
                        });
            ...
            function cargarProcesos2() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        let options = '<option value="">Seleccione</option>';
                        procesos.forEach(function(pro) {
                            options += `<option value="${pro.pro_id}">${pro.pro_id} - ${pro.materiales} / ${pro.pt_descripcion} </option>`;
                        });
