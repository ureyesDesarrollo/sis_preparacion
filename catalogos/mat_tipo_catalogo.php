...
{
	$("#formTipoMat").submit(function(){
		var formData = $(this).serialize();
		$.ajax({
			url: "mat_tipo_insertar.php",
			type: 'POST',
			data: formData,
			success: function(result) {
			  data = JSON.parse(result);
			  //alert("Guardo el registro");
			  alertas("#alerta-errorTipoAlta", 'Listo!', data["mensaje"], 1, true, 5000);
			  $('#formTipoMat').each (function(){this.reset();});  
			}
		});
		return false;
	});
...
function fnc_baja(id){
	...
	if (respuesta){
	$.ajax({
	  url: 'mat_tipo_baja.php',
	  data: 'id=' + id,
	  type: 'post',
	  success: function(result){
		data = JSON.parse(result);
		alertas("#alerta-errorTipoBaja", 'Listo!', data["mensaje"], 1, true, 5000); 
...
<div class="container">
	...
	<div class="col-md-5 col-sm-12">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active" aria-current="page">Origen material</li>
	...
	<div class="col-sm-1 col-md-1">
		<a class="iconos" href="formatos/listado_mat_tipo.php" target="_blank"><img src="../iconos/printer.png" alt="">
		Imprimir</a>
	</div>
	<div class="col-sm-1 col-md-1">
		<a class="iconos"  href="exportar/mat_tipo.php" target="_blank"><img src="../iconos/excel.png" alt="">
		Exp.excel</a>
		<!--<button type="submit" id="export_data" name="export_data" value="Export to excel" class="btn btn-info">Exportar a Excel</button>-->
	</div>
	<?php if(fnc_permiso($_SESSION['privilegio'], 8, 'upe_agregar' ) == 1){?>
	<div class="col-sm-1 col-md-1">
		<a class="iconos"  href="#"  data-toggle="modal" data-target="#ModalAlta" data-whatever="@getbootstrap"><img src="../iconos/tipomat.png" alt="">Origen material</a>
	</div>
	<?php }?>
</div>
<?php include "mat_tipo_listado.php";?>
