...
for ($i = 1; $i < $hdd_cont; $i++) {
	...
	if ($chk_inv != '' && $comentarios != '') {
		//echo "inv=".$chk_inv."<br>";
		$result = mysqli_query($cnx, "UPDATE inventario SET inv_enviado = 4, inv_observaciones = '$comentarios' WHERE inv_id = '$chk_inv' ") or die(mysqli_error($cnx) . "Error al dar de baja");
		...
		mysqli_query($cnx, "INSERT INTO inventario_diario_materiales (usu_id, idm_fecha, idm_documento, mat_id, idm_cant_ing, idm_cant_ant, idm_cant_new, inv_id)VALUES(" . $_SESSION['idUsu'] . ", '" . date("Y-m-d h:i:s") . "', 'Devolver material', '$reg_mat[mat_id]', '$reg_mat[inv_kg_totales]', '$reg_mat[mat_existencia]', '$flt_existencia', '$chk_inv' )") or die(mysqli_error($cnx) . " Error al insertar en el diario");
...
if ($ban == 'ok') {
	$respuesta = array('mensaje' => "Material devuelto");
