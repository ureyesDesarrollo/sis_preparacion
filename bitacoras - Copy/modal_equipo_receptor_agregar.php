...
if ($cnx) {
	...
	$lote = mysqli_query($cnx, "INSERT INTO lotes_anio(lote_fecha, lote_hora, lote_anio,lote_mes, lote_folio, lote_turno, usu_id) 
	VALUES ('" . date("Y-m-d") . "', '" . date("H:i:s") . "','" . date("Y") . "', '" . date("m") . "', '$txt_lote', '$txt_turno', '" . $_SESSION['idUsu'] . "') ") or die(mysqli_error($cnx) . " Error al insertar el lote equipo libre general");

	$reg_ultimo_id = mysqli_fetch_array(mysqli_query($cnx, "select MAX(lote_id) as res from lotes_anio"));
	...
	VALUES('$txtPro', '$cbxEquipo')") or die(mysqli_error($cnx) . " Error al insertar equipos");
		...
		//actualiza lote 
		mysqli_query($cnx, "UPDATE procesos_agrupados SET lote_id = '" . $reg_ultimo_id['res'] . "' where pa_id = '$reg_pa_g[pa_id]' ") or die(mysqli_error($cnx) . " Error al actualizar procesos juntos");
		...
		//actualiza nuevo dato agrupador y lote 
		mysqli_query($cnx, "UPDATE procesos_agrupados SET pa_id = '$reg_pa[pa_id]', lote_id = '" . $reg_ultimo_id['res'] . "' where pa_id = '$reg_pa2[pa_id]' ") or die(mysqli_error($cnx) . " Error al actualizar procesos juntos");
