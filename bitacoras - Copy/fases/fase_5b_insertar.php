...
if ($txtFeIni != '') {
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_5b_g(pro_id, pe_id, pfg5_temp_ag, pfg5_temp, pfg5_acido, pfg5_termina, pfg5_temp2, pfg5_acido_fuerte, usu_id) VALUES('$hdd_pro_id', '$hdd_pe_id', '$txtTemp', '$txtTemp2', '$txtAcido', '$txtTermina', '0', '0', '" . $_SESSION['idUsu'] . "' ) ") or die(mysqli_error($cnx) . " Error al insertar 2");
	...
	for ($i = 1; $i <= 8; $i++) {
		...
		if ($txtAcidoF != '' and $txtPhF != '') {
			mysqli_query($cnx, "INSERT INTO procesos_fase_5b_d(pfg5_id, pfd5_ren, pfd5_acido, pfd5_ph, pfd5_temp, usu_id, pfd5_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen', '$txtAcidoF', '$txtPhF', '0', '" . $_SESSION['idUsu'] . "', SYSDATE() ) ") or die(mysqli_error($cnx) . " Error al insertar " . $i);
	...
	if ($strMsj != '') {
		...
		if ($txtFeTerm != '') {
			...
			mysqli_query($cnx, "UPDATE procesos_fase_5b_g SET pfg5_cocido_ph = '0', taa_id = '0', pfg5_ce_agua = '0', pfg5_ph_agua = '0', pfg5_ce = '0', pfg5_hr_reales = '0' WHERE pfg5_id = '$hdd_pfg' ") or die(mysqli_error($cnx) . " Error al actualizar 4g");
