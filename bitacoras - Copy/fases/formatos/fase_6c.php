...
$sqlfg = mysqli_query($cnx, "SELECT * FROM procesos_fase_6b_g as pfg INNER JOIN procesos as p  on(pfg.pro_id=p.pro_id) WHERE p.pro_id  = '$idx_pro' AND pfg.pe_id = 16") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_6b_g ");
$regfg = mysqli_fetch_assoc($sqlfg);

if ($regfg['tpa_id'] != '') {
  $sqlTagua = mysqli_query($cnx, "SELECT * FROM tipos_agua  WHERE tpa_id= $regfg[tpa_id]") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_6b_g ");
  $regTagua = mysqli_fetch_assoc($sqlTagua);
}

if ($regfg['pro_id'] != '') {
  $sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '16'") or die(mysqli_error($cnx) . "Error: en consultar procesos auxiliar");
  $regProAux = mysqli_fetch_assoc($sqlProAux);
}


if ($regfg['pro_id'] != '') {
  $sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '16'") or die(mysqli_error($cnx) . "Error: en consultar procesos de liberacion");
  ...
  <table width="100%" style="margin:20px 0px 20px 0px">
    ...
    <tr>
      <td width="70%" valign="top">
        <table  border="1" style="margin-top: 10px;width:98%">
          <tr style="font-weight: bold;background: #e6e6e6">
            ...
            <td width="17%" style="font-size: small; text-align: center;">Agua a</td>
            <td style="font-size: small; text-align: center;">Observaciones</td>
            <td style="font-size: small; text-align: center;">Capturo</td>

          </tr>
          <?php
          $sqlfd = mysqli_query($cnx, "SELECT * FROM procesos_fase_6b_d  WHERE pfg6_id = '$regfg[pfg6_id]'") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_6b_d");
      ...
      <td rowspan="3" valign="top">
        ...
        <table class="liberacion" style="width:100%" border="1">
          <tr>
            <td width="58" rowspan="5" style="font-size:50px">6c</td>
            <td colspan="2" style="background: #e6e6e6;font-weight: bold;">LIBERACION CE A <?php echo fnc_rango_de(16) ?> a <?php echo fnc_rango_a(16) ?> hora) MAX</td>
          </tr>
          <tr>
            <td width="107">Ce liberación</td>
            <td width="40"><?php echo $regProLib['prol_ce'] ?></td>
          </tr>
          <tr>
            <td>Horas totales</td>
      ...
      <td valign="top">
        <table border="1" width="98%">
          <tr style="font-weight: bold;">
            ...
            <td width="190" rowspan="2" style="background: #e6e6e6">(<?php echo fnc_hora_de(16) ?> a <?php echo fnc_hora_a(16) ?> horas)</td>
          ...
          <tr>
            ...
            <!--<td><?php echo $regfg['pfg6_agua_a'] ?></td>-->
