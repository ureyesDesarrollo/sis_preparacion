...
//$id_l = $_GET['id_l'];
//$sqlf1g = mysqli_query($cnx, "SELECT * FROM procesos_fase_1_g as pf1g INNER JOIN procesos as p on(pf1g.pro_id=p.pro_id) AND p.pl_id='1' AND p.pro_estatus='1'") or die(mysqli_error($cnx)."Error: en consultar procesos_fase_1_g ");
$sqlf1g = mysqli_query($cnx, "SELECT * FROM procesos_fase_1_g as pf1g INNER JOIN procesos as p  on(pf1g.pro_id=p.pro_id) WHERE p.pro_id  = '$idx_pro' AND pf1g.pe_id = 31") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_1_g ");
$regf1g = mysqli_fetch_assoc($sqlf1g);


//$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar as pa INNER JOIN usuarios as u on(pa.usu_sup=u.usu_id) WHERE pro_id = '$regf1g[pro_id]'") or die(mysqli_error($cnx)."Error: en consultar procesos auxiliar");
$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regf1g[pro_id]' AND pe_id = '31'") or die(mysqli_error($cnx) . "Error: en consultar procesos auxiliar");
$regProAux = mysqli_fetch_assoc($sqlProAux);


//$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion as prolib INNER JOIN usuarios as u on(prolib.usu_id=u.usu_id) WHERE pro_id = '$regf1g[pro_id]'") or die(mysqli_error($cnx)."Error: en consultar procesos de liberacion");
$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$regf1g[pro_id]' AND pe_id = '31'") or die(mysqli_error($cnx) . "Error: en consultar procesos de liberacion");
    ...
    <tr>
      <td>
        <table border="1" style="background: #FCEFF2;font-size: 12px;width: 100%">
          <tr>
            <td height="45" colspan="10"> Lavados de carga<!--  Este proceso se puede hacer con aguar recuperada limpia (pila 1). -->
              <!--   Lavados finales de paleto a paleto y en el ultimo lavado utilizar agua limpia si se necesita bajar CE -->
            </td>
          </tr>
        </table>
      </td>
      ...
      <td>
        <table width="100%" border="1">
          <tr>
            ...
            <td style="font-weight: bold;background: #e6e6e6">PH agua de inicio</td>
            <td style="font-weight: bold;background: #e6e6e6">CE agua de inicio</td>
            <td style="font-weight: bold;background: #e6e6e6">Temp agua inicio</td>
            <td style="font-weight: bold;background: #e6e6e6">Litros peroxido</td>
            <td width="14%" rowspan="2" style="background: #e6e6e6;font-weight: bold;">Bajar CE A 4.0</td>
          </tr>
          <tr>
            <td><?php echo fnc_formato_fecha($regProAux['proa_fe_ini']) ?></td>
            <td><?php echo $regProAux['proa_hr_ini'] ?></td>
            <td><?php echo $regf1g['pfg1_ph_agua'] ?></td>
            <td><?php echo $regf1g['pfg1_ce_agua'] ?></td>
            <td><?php echo $regf1g['pfg1_temp_ag'] ?></td>
            <td><?php echo $regf1g['pfg1_peroxido'] ?></td>
    ...
    <tr>
      <td>
        <table width="100%" border="1">
          ...
          <?php
          $sqlf1d = mysqli_query($cnx, "SELECT * FROM procesos_fase_1_d as pf1d  inner join tipos_agua as ta on(pf1d.tpa_id= ta.tpa_id) WHERE pf1d.pfg1_id = '$regf1g[pfg1_id]'") or die(mysqli_error($cnx) . "Error: en consultar el tipo de material");
          $regf1d = mysqli_fetch_assoc($sqlf1d);


          do {

            $agua_a = mysqli_query($cnx, "SELECT * FROM tipos_agua_a WHERE taa_id = '$regf1d[taa_id]'") or die(mysqli_error($cnx) . "Error: en consultar el tipo de agua a");
      ...
      <td>
        <table border="1" width="100%">
          <tr style="font-weight: bold;">
            ...
            <td style="background: #e6e6e6">Redox</td>
            <td style="background: #e6e6e6">Usuario</td>
            <td style="border-right:1px solid#fff;border-top:1px solid#fff;border-bottom:1px solid#fff;background: #e6e6e6"><?php echo fnc_hora_de(1) ?> a <?php echo fnc_hora_a(1) ?> horas totales</td>
            <td width="7%" style="border-bottom:1px solid#fff;border-top:1px solid#fff"></td>
            <td rowspan="5" width="30" style="font-size:50px">1c</td>
            <td colspan="3" style="background: #e6e6e6;font-weight: bold;">Liberación CE a 4.0 MAX</td>
          </tr>
          <tr>
            <td><?php echo fnc_formato_fecha($regProAux['proa_fe_fin']) ?></td>
            <td><?php echo $regProAux['proa_hr_fin'] ?></td>
            <td><?php echo $regf1g['pfg1_redox'] ?></td>
