...
$sqlfg = mysqli_query($cnx, "SELECT * FROM procesos_fase_8_g as pfg INNER JOIN procesos as p  on(pfg.pro_id=p.pro_id) WHERE p.pro_id  = '$idx_pro'") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_8_g ");
$regfg = mysqli_fetch_assoc($sqlfg);

$cad_pro_con = mysqli_query($cnx, "SELECT hrs_totales_calculadas, hrs_totales_capturadas, pro_hrs_tot_muerto  FROM procesos WHERE pro_id = '$regfg[pro_id]'");
$reg_pro_con = mysqli_fetch_array($cad_pro_con);

$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '20'") or die(mysqli_error($cnx) . "Error: en consultar procesos auxiliar");
$regProAux = mysqli_fetch_assoc($sqlProAux);

$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion_b WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '20'") or die(mysqli_error($cnx) . "Error: en consultar procesos de liberacion");
...
<div class="divProcesos">
  <table width="100%" style="margin:20px 0px 20px 0px">
    <tr>
      <td colspan="2">
        <table width="90%" border="1" style="background: #FCEFF2;font-size: 12px;width: 99%">
          <tr>
            <td height="45" colspan="14">LAVADOS FINALES
              <div></div>
            </td>
          </tr>
        </table>
    ...
    <tr>
      <td width="100%" colspan="2">
        <table width="99%" border="1" style="margin-top: 10px">
          ...
          <?php
          $sqlfd = mysqli_query($cnx, "SELECT * FROM procesos_fase_8_d WHERE pfg8_id = '$regfg[pfg8_id]'") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_8_d");
          ...
          do {
            ...
            if ($regfd['pfd8_ren'] == 1 || $regfd['pfd8_ren'] == 3) {
              $nivel = "er";
            }
            if ($regfd['pfd8_ren'] == 2) {
              $nivel = "do";
            }
            if ($regfd['pfd8_ren'] == 4 || $regfd['pfd8_ren'] == 5) {
              $nivel = "to";
            }
          ?>
            <tr>
              <td><?php echo $regfd['pfd8_ren'] . $nivel . " Lav" ?></td>
    ...
    <tr>
      <td width="60%" valign="top">
        <table width="98%" border="1">
          <tr style="font-weight: bold;">
            <td width="223" style="background: #e6e6e6">Fecha termina lavados finales</td>
            <td width="116" style="background: #e6e6e6">Hora termina </td>
            <td width="101" style="background: #e6e6e6">Horas totales</td>
            <td width="102" style="background: #e6e6e6">Usuario</td>
            <td width="103" rowspan="2" style="background: #e6e6e6">(<?php echo fnc_hora_de(20) ?> a <?php echo fnc_hora_a(20) ?> horas)</td>
          </tr>
          <tr>
      ...
      <td width="20%" rowspan="3" valign="top">
        <table border="1">
          <tr>
            <td width="36" rowspan="14" style="font-size:50px">8</td>
            <td colspan="4" style="background: #e6e6e6;font-weight: bold;">LIBERACION PH LIBERACIÓN (PH <?php echo fnc_rango_de(20) ?> a <?php echo fnc_rango_a(20) ?>)</td>
          </tr>
      ...
      <td valign="top">
        ...
        <table width="98%" border="1">
          <tr style="font-weight: bold;">
            ...
            <td width="124" rowspan="2" style="background: #e6e6e6">(67 A 92 HORAS)</td>
          </tr>
          <tr>
            <td><?php echo $reg_pro_con['hrs_totales_capturadas'] ?></td>
            <td><?php echo $reg_pro_con['pro_hrs_tot_muerto'] ?></td>
            <td><?php echo fnc_nom_usu($usu_aux) ?></td>
          </tr>
        </table>
        <p>&nbsp;</p>
        <table width="98%" border="1">
          <tr style="font-weight: bold;">
            <td width="167" style="background: #e6e6e6">Fecha sale a producción</td>
            <td width="115" style="background: #e6e6e6">Hora </td>
            <td width="128" style="background: #e6e6e6">Usuario</td>
          </tr>
          <tr>
    ...
    <tr <?php echo $oculta_opciones ?>>
      <td colspan="2">
        ...
        ?>
            ...
            <button style="float: right;" <?php echo $oculta_opciones ?> type="button" class="btn btn-success" id="paleto" onClick="javascript:abre_modal_equipo_receptor(<?php echo $regfg['pro_id'] ?>, <?php echo $id_e ?>);"> <img src="../iconos/procesos2.png" alt="">Equipo
            </button>
        <?php }
        } ?>
      </td>
