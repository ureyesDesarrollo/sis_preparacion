...
if ($txtFeIni != '' and $txtHrIni != ''  and $txtPhAnt != '' and $txtCe != '' and $txtAjSosa != '' and $txtPhAj != '' and  $txtPeroxido != '' and $txtPhAnt != ' ' and $txtCe != ' ') {
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_2_g(pro_id, pe_id, pfg2_temp_ag, pfg2_ph_ant, pfg2_ce, pfg2_sosa, pfg2_ph_aju, pfg2_peroxido) VALUES('$hdd_pro_id', '$hdd_pe_id', 0, '$txtPhAnt', '$txtCe', '$txtAjSosa', '$txtPhAj', '$txtPeroxido' ) ") or die(mysqli_error($cnx) . " Error al insertar 2");

	$respuesta = array('mensaje' => "Fase 2e agregada");
} else {
	for ($i = 1; $i <= 16; $i++) {
		$txtRen = ${"txtRen" . $i};
		$txtHr = ${"txtHr" . $i};
		...
		//if ($txtHr != '' and $txtPh != '' and $txtSosa != '' and $txtTemp != '' and $txtPeroxido != '') // and $txtRedox != ''
		...
		/* 	fnc_alertas($hdd_pe_id, 'pH', $hdd_pro_id, $txtPh, $_SESSION['idUsu'], $txt_equipo, $txt_equipo, 'R', '0', '0');
			if ($txtRen <= '4') {
				fnc_alertas($hdd_pe_id, 'ppm', $hdd_pro_id, $txtRedox, $_SESSION['idUsu'], $txt_equipo, $txt_equipo, 'R', '0', '0');
			} */
		}
	}
	...
	if ($strMsj != '') {
		...
		if ($txtFeTerm != '') {
			...
			$strMsj = "Fase 2e actualizada";
		} else {
			if ($txtHrTotales != '') {
				mysqli_query($cnx, "INSERT INTO procesos_liberacion (usu_id, pro_id, pe_id, prol_hr_totales, prol_ph, prol_color ) VALUES('" . $_SESSION['idUsu'] . "', '$hdd_pro_id', '$hdd_pe_id', '$txtHrTotales', '$txtPhLib', '$cbxColor') ") or die(mysqli_error($cnx) . " Error al insertar L");

				$strMsj = "Fase 2e parametros capturados";
			} else {
				$strMsj = "Esta vacio";
			}
		}
