...
for ($i = 1; $i <= 10; $i++) {
	...
	if ($txtIniLlen != '' or $txtFinLlen != '' or $txtIniDren != '' or $txtFinDren != '' or $txtPh != '' or $txtCe != '') {
		//if($cbxTipAgd != ''){$str_update .= ",tpa_id = '$cbxTipAgd'";}
		//if($txtMov != ''){$str_update .= ", pfd8_mov = '$txtMov'";}
		if ($txtIniLlen != '') {
			$str_update .= ", pfd8_ini_llenado = '$txtHIniDrenado'";
		}
		if ($txtFinLlen != '') {
			$str_update .= ", pfd8_fin_llenado = '$txtHFinDrenado'";
		}
		if ($txtIniDren != '') {
			$str_update .= ", pfd8_ini_dren = '$txtHIniDrenado'";
		}
		if ($txtFinDren != '') {
			$str_update .= ", pfd8_fin_dren = '$txtHFinDrenado'";
		}
		if ($txtPh != '') {
			$str_update .= ", pfd8_ph = '$txtPh'";
		}
		if ($txtCe != '') {
			$str_update .= ", pfd8_ce = '$txtCe'";
		}
		//if($txtTemp != ''){$str_update .= ", pfd8_temp = '$txtTemp'";}
		//if($txtagua_a != ''){$str_update .= ",taa_id = '$txtagua_a'";}
		if ($txtObs != '') {
			$str_update .= ", pfd8_observaciones = '$txtObs' ";
		}
