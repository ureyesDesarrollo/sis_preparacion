...
if($txtFeIni != '')
{
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_4b_g(pro_id, pe_id, pfg4_temp_ag, usu_id) VALUES('$hdd_pro_id', '$hdd_pe_id', '0', '".$_SESSION['idUsu']."' ) ") or die(mysqli_error($cnx)." Error al insertar 2");
...
else
{
	for($i = 1; $i <= 10; $i++)
	{
		...
		{ 
			...
			{
				mysqli_query($cnx, "UPDATE procesos_fase_4b_d SET pfd4_ppm = '$txtPpm' WHERE pfd4_id = '$hddRen' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);
				$strMsj = "Se modifico el renglon ".$i;
			}
	...
	else
	{
		if($txtFeTerm != '')
		{
			...
			mysqli_query($cnx, "UPDATE procesos_fase_4b_g SET taa_id = null, pfg4_horas_reales = '$txtHrsReales' WHERE pfg4_id = '$hdd_pfg' ") or die(mysqli_error($cnx)." Error al actualizar 4g");

			$strMsj = "Fase 4d actualizada";
		}
		else
		{
			if($txtHrTotales != '')
			{
				...
				$strMsj = "Fase 4d parametros capturados";
			}
			else
			{
				$strMsj = "Esta vacio";
