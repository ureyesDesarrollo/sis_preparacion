...
$reg_inv = mysqli_fetch_array($cad_inv);
 ...
 //actualiza a enviado
 mysqli_query($cnx, "UPDATE inventario SET inv_kg_totales = '$txt_pelambrado', inv_enviado = '2' WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx)." Error al actualizar el inventario");
...
if ($txtSobra != '0') {		
    ...
    mysqli_query($cnx, "INSERT INTO inventario(inv_fecha, inv_hora, inv_dia, inv_no_ticket, inv_placas, inv_camioneta, prv_id, mat_id, inv_kilos, inv_prueba, inv_desc_ag, inv_desc_d, inv_desc_ren, inv_kg_totales, inv_calidad, inv_no_factura, inv_peso_factura, inv_por_merma, inv_no_tarimas, inv_no_sacos,inv_enviado,inv_fe_enviado,inv_id_key) 
					VALUES('$reg_inv[inv_fecha]', '$reg_inv[inv_hora]', '$reg_inv[inv_dia]','$reg_inv[inv_no_ticket]', '$reg_inv[inv_placas]', '$reg_inv[inv_camioneta]', '$reg_inv[prv_id]', '$reg_inv[mat_id]', '$txt_pelambrado', '$reg_inv[inv_prueba]', '$reg_inv[inv_desc_ag]', '$reg_inv[inv_desc_d]', '$reg_inv[inv_desc_ren]', '$reg_inv[inv_kg_totales]', '$reg_inv[inv_calidad]', '$reg_inv[inv_no_factura]', '$reg_inv[inv_peso_factura]', '$reg_inv[inv_por_merma]', '$reg_inv[inv_no_tarimas]', '$reg_inv[inv_no_sacos]','1','".date("Y-m-d h:i:s")."','$hdd_id')") or die(mysqli_error($cnx)." Error al insertar");

ins_bit_acciones($_SESSION['idUsu'],'E', $id, '3');
}else{
    //actualiza a enviado
    mysqli_query($cnx, "UPDATE inventario SET inv_enviado = '1' WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx)." Error al actualizar el inventario");
