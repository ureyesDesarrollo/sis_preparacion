...
<script>
	...
	function AbreModalConsultar(id) {
		$.ajax({
			...
			success: function(result) {
				$("#modal_consulta_x").html(result);
				$('#modal_consulta_x').modal('show')
			}
		});
		return false;
	};
	...
	<div class="col-md-5 col-sm-12">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active" aria-current="page">Tipos de Proceso</li>
			</ol>
		</nav>
	</div>
	<div class="diviconos">
		...
		<?php if (fnc_permiso($_SESSION['privilegio'], 9, 'upe_agregar') == 1) { ?>
			<div class="col-sm-1 col-md-1">
				<a class="iconos" href="#" data-toggle="modal" data-target="#ModalAlta" data-whatever="@getbootstrap"><img src="../iconos/procesos.png" alt="">
					Tipo proceso</a>
			</div>
		<?php } ?>
	</div>
	<?php include "tipo_proceso_listado.php"; ?>

	<div class="modal fade" id="ModalAlta" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<?php include "tipo_proceso_alta.php"; ?>
	</div>

	<div class="modal" id="modalEditar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false">
	</div>

	<div class="modal" id="modal_consulta_x" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false">
	</div>
</div>
