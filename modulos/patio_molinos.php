...
WHERE a.ac_ban = 'P' AND i.inv_tomado = 0") or die(mysqli_error($cnx) . "Error: en consultar inventario");
...
<head>
    ...
    <title>Patio molinos - <?php echo date("d-m-Y"); ?></title>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-red bg-red" id="encabezado">
        <div class="container-fluid">
            ...
            <a class="navbar-brand" href="#">PATIO MOLINOS</a>
            ...
            <div class="collapse navbar-collapse" id="navbarText">
                ...
                <span class="navbar-text">
                    <a class="nav-link active" href="mp_croquis.php"><i class="fa-solid fa-splotch"></i> Patio MP</a>
                </span>
                <span class="navbar-text">
    ...
    <div class="container-fluid" id="full">
        <div class="row" id="indicadores">
            <div class="col-sm-12 col-md-12">
                <?php echo "<b>INVENTARIO TOTAL " . number_format($reg_tot_inv['total_inventario']) . "</b>"; ?>
            </div>
            <div class="col-sm-12 col-md-12">
        ...
        <div class="table-responsive">
            <table>
                ...
                $total_cajon = mysqli_query($cnx, "SELECT * FROM almacen_cajones WHERE ac_ban='P' AND ac_estatus = 'A' ORDER BY ac_descripcion") or die(mysqli_error($cnx) . "Error: en consultar el material");
                ...
                for ($i = 0; $i < $num_renglones; $i++) {
                    ...
                    /* ---------------------NO. CAJONES------------------------ */
                    $num_cajon = mysqli_query($cnx, "SELECT * FROM almacen_cajones WHERE ac_ban='P' AND ac_estatus = 'A' ORDER BY ac_descripcion LIMIT $valor") or die(mysqli_error($cnx) . "Error: en consultar el material");
                    $reg_num_cajon = mysqli_fetch_assoc($num_cajon);

                    echo '<tr>';
                    do {
                        if (fnc_permiso($_SESSION['privilegio'], 32, 'upe_editar') == 1) {
                            echo '<th id="cajones"><a id="cajon" href="#" onClick="javascript:modal_cajon_editar_molinos(' . $reg_num_cajon['ac_id'] . ')">' . $reg_num_cajon['ac_descripcion'] . '</a></th>';
                        } else {
                            echo '<th id="cajones">' . $reg_num_cajon['ac_descripcion'] . '</th>';
                        }
                    ...
                    /* -----------------------DETALLE-------------------------- */
                    echo '<tr>';
                    $detalle_cajon = mysqli_query($cnx, "SELECT * FROM almacen_cajones WHERE ac_ban='P' AND ac_estatus = 'A' ORDER BY ac_descripcion LIMIT $valor") or die(mysqli_error($cnx) . "Error: en consultar el material");
                    $reg_detalle = mysqli_fetch_assoc($detalle_cajon);

                    if (mysqli_num_rows($detalle_cajon) > 0) {
                        do {
                            $cad_inv = mysqli_query($cnx, "SELECT p.prv_nombre,p.prv_ncorto,m.mat_nombre,i.inv_kg_totales,i.inv_folio_interno,i.inv_fecha,i.inv_hora_entrada,p.prv_id,i.inv_id,i.ac_id  FROM inventario as i
                    INNER JOIN proveedores AS p on(i.prv_id = p.prv_id)
                    INNER JOIN materiales as m on(i.mat_id = m.mat_id)
                    WHERE i.ac_id = " . $reg_detalle['ac_id'] . " AND i.inv_tomado = 0") or die(mysqli_error($cnx) . "Error: en consultar inventario");
                    ...
                    WHERE i.ac_id = " . $reg_detalle['ac_id'] . " AND i.inv_tomado = 0 AND inv_enviado <> 3 AND inv_enviado <> 4 ") or die(mysqli_error($cnx) . "Error: en consultar inventario");
                            ...
                            do {
                                if (mysqli_num_rows($cad_inv) > 0) {
                                    ...
                                    echo $reg_inv['inv_folio_interno'] . "<br>";
                                    echo $fecha . ' ' . $color . "<br>";
                                    ...
                                    echo '<a href="#" onClick="javascript:modal_cajon_inventario(' . $reg_inv['inv_id'] . ')"  data-bs-toggle="tooltip" data-bs-placement="top" title="Consulta inventario"><i class="fa-solid fa-receipt fa-2xl" style="color: #bfc1c3;"></i></a>' . "<br><br>";
                                }
        ...
        <!-- Modal edición cajon -->
        <div class="modal fade" id="modal_cajon_edicion_molinos" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
        </div>
...
<script>
    ...
    /*Abrir modal edición*/
    function modal_cajon_editar_molinos(cajon) {
        $.ajax({
            type: 'post',
            ...
            success: function(result) {
                $("#modal_cajon_edicion_molinos").html(result);
                $('#modal_cajon_edicion_molinos').modal('show')
            }
        });
        return false;
    };
