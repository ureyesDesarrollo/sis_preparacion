 <?php
  ...
  $cad_pelambrado = mysqli_query($cnx, "SELECT ip_ban 
  FROM inventario_pelambre WHERE ep_id = '" . $_POST['hdd_id'] . "' AND ip_ban = 1");
  $reg_pelambrado = mysqli_fetch_array($cad_pelambrado);
  ?>
 <script>
   $(document).ready(function() {
     $("#formEtapasEditar").submit(function() {
       ...
       $.ajax({
         url: "estatus_actualizar_eq_pelambre.php",
         type: 'POST',
         data: formData,
           ...
           <div class="col-md-4">
             ...
             <select onchange="campo_ot()" name="cbxEstatus" class="form-control is-valid" id="cbxEstatusAct" placeholder="" disabled>
               <?php
                ...
                ?>
                 <option value="<?php echo mb_convert_encoding($reg_est['le_id'], "UTF-8");  ?>" <?php if (mb_convert_encoding($reg_est['le_id'], "UTF-8") == $registros['le_id']) { ?>selected="selected" <?php } ?>>
                   <?php echo mb_convert_encoding($reg_est['le_estatus'] . ' - ' . $reg_est['le_id'], "UTF-8");  ?>
                 </option>
               <?php } ?>
             </select>
           ...
           <div class="col-md-4">
             ...
             <select onchange="campo_ot()" name="cbxEstatus" class="form-control is-valid" id="cbxEstatus" placeholder="" required>
               ...
               <?php
                ...
                if ($registros['le_id'] == 13 and !isset($reg_pelambrado['ip_ban'])) {
