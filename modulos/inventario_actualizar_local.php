...
//Insera el nuevo inventario que se dividio, con estatus recibido y tomado en 0
mysqli_query($cnx, "INSERT INTO inventario(inv_fecha, inv_hora, inv_dia, inv_no_ticket, inv_placas, inv_camioneta, prv_id, mat_id, inv_kilos, inv_prueba, inv_desc_ag, 
                    inv_desc_d, inv_desc_ren, inv_kg_totales, inv_calidad, inv_no_factura, inv_peso_factura, inv_por_merma, inv_no_tarimas, inv_no_sacos,inv_enviado,
                    inv_fe_enviado,inv_fe_recibe,prv_recibe,inv_tomado,inv_kg_entrada_maq,inv_kg_lavador,int_cve_compra ,inv_total_cueros,inv_prueba2,inv_desc_ag2,
                    inv_desc_d2,inv_desc_ren2, inv_id_key) 
                    VALUES('$reg_inv[inv_fecha]', '$reg_inv[inv_hora]', '$reg_inv[inv_dia]','$reg_inv[inv_no_ticket]', '$reg_inv[inv_placas]', '$reg_inv[inv_camioneta]',
                    '$reg_inv[prv_id]', '$reg_inv[mat_id]', '$reg_inv[inv_kilos]', '$txtPrbSecador', '$txtDAgua', '$txtDescarne', '$txtDRendimiento', '$txtKgTotales', 
                    '$reg_inv[inv_calidad]', $reg_inv[inv_no_factura], '$reg_inv[inv_peso_factura]', '$reg_inv[inv_por_merma]', '$reg_inv[inv_no_tarimas]', 
                    '$reg_inv[inv_no_sacos]','2',$reg_inv[inv_fe_enviado], '".date("Y-m-d H:i:s")."' ,'$cbxProveedor','0','$txtKgEntradaMaq','$txtKgLavador',
                    '$reg_inv[int_cve_compra]','$reg_inv[inv_total_cueros]','$txtPrbSecador','$txtDAgua','$txtDescarne','$txtDRendimiento','$hdd_id')") or die(mysqli_error($cnx)." Error al insertar nuevo inventario");
}else{
    mysqli_query($cnx, "UPDATE inventario SET inv_enviado = '2', inv_kg_totales = '$txtKgTotales',inv_fe_recibe = '".date("Y-m-d H:i:s")."', prv_id = '$reg_inv[prv_id]',prv_recibe = '$cbxProveedor',inv_kg_entrada_maq = '$txtKgEntradaMaq',
                        inv_kg_lavador = '$txtKgLavador', inv_prueba2 = '$txtPrbSecador', inv_desc_ag2 = '$txtDAgua', inv_desc_d2 = '$txtDescarne',inv_desc_ren2 = '$txtDRendimiento'
                        WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx)." Error al actualizar el inventario");
}
...
//$flt_existencia = $reg_mat['mat_existencia'];
