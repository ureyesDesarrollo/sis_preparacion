...
//if ($txtSobra != '0') {
    ...
    //cajon, estatus
    mysqli_query($cnx, "INSERT INTO inventario(inv_fecha, inv_hora, inv_dia, inv_no_ticket, inv_placas, inv_camioneta, prv_id, mat_id, inv_prueba, inv_desc_ag, inv_desc_d, inv_desc_ren, inv_calidad, inv_no_factura, inv_peso_factura, inv_por_merma, inv_no_tarimas, inv_no_sacos,inv_enviado,inv_fe_enviado,inv_id_key,prv_recibe,inv_folio_interno,inv_folio_interno2,inv_hora_entrada,inv_hora_salida, ac_id, inv_solicitado, inv_kilos, inv_kg_totales ) 
					VALUES('$reg_inv[inv_fecha]', '$reg_inv[inv_hora]', '$reg_inv[inv_dia]','$reg_inv[inv_no_ticket]', '$reg_inv[inv_placas]',
					'$reg_inv[inv_camioneta]', '$reg_inv[prv_id]', '$reg_inv[mat_id]', '$reg_inv[inv_prueba]', '$reg_inv[inv_desc_ag]',
					'$reg_inv[inv_desc_d]', '$reg_inv[inv_desc_ren]', '$reg_inv[inv_calidad]', '$reg_inv[inv_no_factura]', 
					'$reg_inv[inv_peso_factura]', '$reg_inv[inv_por_merma]', 
					'$reg_inv[inv_no_tarimas]', '$reg_inv[inv_no_sacos]','2',
					'" . date("Y-m-d h:i:s") . "','$hdd_id','',
					'$reg_inv[inv_folio_interno]','$txt_folio_interno',$hora_entrada,$hora_salida,'$cbx_cajon','' , '$reg_inv[inv_kilos]', '$txtEnvia')") or die(mysqli_error($cnx) . " Error al insertar");
    ...
    mysqli_query($cnx, "UPDATE inventario SET inv_enviado = '2', prv_recibe = '', ac_id ='$cbx_cajon', inv_folio_interno2 = '$txt_folio_interno', inv_fe_recibe = '" . date("Y-m-d h:i:s") . "' WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx) . " Error al actualizar el inventario");

    mysqli_query($cnx, "UPDATE inventario_pelambre SET ip_ban = '0' WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx) . " Error al actualizar el inventario");

    //Selecciona pelambre
    $cad_pelambre = mysqli_query($cnx, "SELECT * FROM inventario_pelambre WHERE inv_id = '$hdd_id' ");
    $reg_pelambre = mysqli_fetch_array($cad_pelambre);

    #actualiza estatus equipo a drenado
    mysqli_query($cnx, "UPDATE equipos_preparacion SET le_id = 14 WHERE ep_id = " . $reg_pelambre['ep_id'] . "") or die(mysqli_error($cnx) . " Error al actualizar el inventario");
