...
$cadena = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 WHERE inv_fecha = '$hoy' AND prv_tipo = 'E'") or die(mysql_error()."Error: en consultar el inventario 1");
...
$cadena2 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fecha = '$hoy' AND prv_tipo = 'L'") or die(mysql_error()."Error: en consultar el inventario 2");
...
/*$cadena3 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fe_recibe = '$hoy' and i.inv_enviado = 2") or die(mysql_error()."Error: en consultar el inventario 2");
...
$cadena3 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE substring(inv_fe_recibe,1,10) = '$hoy' and i.inv_enviado = 2") or die(mysql_error()."Error: en consultar el inventario 2");
$registros3 = mysqli_fetch_assoc($cadena3);
...
$cad_scat = mysqli_query($cnx, "SELECT DISTINCT t.mt_id, t.mt_descripcion
  ...
  WHERE i.inv_fecha = '$hoy' "); 
$reg_scat = mysqli_fetch_array($cad_scat);

header('Content-type: application/vnd.ms-excel');
header("Content-Disposition: attachment; filename=inventario_".$hoy.".xls");
      ...
      <div class="tablehead" style="width: 1200px">
       <table width="" style="border: 1px solid #fff; background:#e6e6e6">
        <tr style="border: 1px solid #e6e6e6">
         ...
         <td style="border: 1px solid #fff"><p>FECHA:<?php echo $hoy; ?></p>
    ...
    <p>&nbsp;</p>
     ...
     <tbody>
      ...
      do{
        ...
        if ($registros3['inv_id'] != '') {
          $cad_inv3 = mysqli_query($cnx, "SELECT inv_kg_totales FROM `inventario` WHERE inv_id = ".$registros3['inv_id']."") or die(mysqli_error());
          $reg_inv3 = mysqli_fetch_assoc($cad_inv3);

          $cad_sum3 = mysqli_query($cnx, "SELECT SUM(inv_kg_totales) as inv_kg_totales  FROM `inventario` WHERE  inv_id_key = ".$registros3['inv_id']."") or die(mysqli_error());
        ...
        <tr style="border: 1px solid">
          ...
          <td><?php echo $registros3['inv_no_ticket'];//."-".$registros3['inv_id'] ?></td>
          ...
          <table>
            ...
            <?php do { 
              ...
              $inv_obj = mysqli_query($cnx, "SELECT mto_kilos as res
                ...
                WHERE mt_id = '$reg_scat[mt_id]' and mto_fecha = '$hoy'
              ...
              $inv = mysqli_query($cnx, "SELECT sum(i.inv_kg_totales)as res2 
                FROM inventario as i 
                INNER JOIN materiales as m  ON(i.mat_id = m.mat_id)
                WHERE m.mt_id = '$reg_scat[mt_id]' and inv_fecha = '$hoy'
                ") or die(mysql_error()."Error: en consultar el inventario 3");
        ...
        <td width="48%" valign="top"><table border="1" cellspacing="0" cellpadding="0" style="font-size:14px; color:#666666" width="554">
            ...
            <tr style="background: #32383e;color: #fff">
              ...
              <tr>
                ...
                <td rowspan="2">MIN. 28 </td>
                <td rowspan="2">MIN. 28 </td>
                <td rowspan="2">MIN. 30 </td>
