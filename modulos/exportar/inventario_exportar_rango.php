...
$cadena = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 WHERE inv_fecha >= '$fechaIni' AND inv_fecha <= '$fechaFin' AND prv_tipo = 'E' AND inv_id_key is NULL") or die(mysqli_error($cnx) . "Error: en consultar el inventario 1");
...
$cadena2 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fecha >= '$fechaIni' AND inv_fecha <= '$fechaFin' AND prv_tipo = 'L'") or die(mysqli_error($cnx) . "Error: en consultar el inventario 2");
...
$cadena3 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fe_recibe >= '$fechaIni' and inv_fe_recibe <= '$fechaFin' and i.inv_enviado = 2") or die(mysqli_error($cnx) . "Error: en consultar el inventario 2");
      ...
      <div class="tablehead" style="width: 1200px">
        <table width="" style="border: 1px solid #fff; background:#e6e6e6">
          <tr style="border: 1px solid #e6e6e6">
            ...
            <td colspan="11" align="center" style="border: 1px solid #e6e6e6">
              <h1>ENTRADAS DE MATERIA PRIMA</h1>
            </td>
            <td style="border: 1px solid #fff">
              <p>FECHA:<?php echo $fechaIni . ' al ' . $fechaFin; ?></p>
              <p>REF F 001-REV.000</p>
            </td>
          </tr>
        </table>
        <p>&nbsp;</p>
      ...
      <table width="100%">
        <tr>
          <td width="41%" valign="top">
            <table>
              <tr style="background: #32383e;color: #fff">
                <td colspan="5" align="center">Entrada cuero del día</td>
              </tr>
              <tr style="background: #32383e;color: #fff">
              ...
              <?php do {
                ...
                WHERE mt_id = '$reg_scat[mt_id]' and mto_fecha >= '$fechaIni' and mto_fecha <= '$fechaFin'
                ...
                FROM inventario as i 
                INNER JOIN materiales as m  ON(i.mat_id = m.mat_id)
                WHERE m.mt_id = '$reg_scat[mt_id]' and inv_fecha >= '$fechaIni' AND inv_fecha <= '$fechaFin'
                ") or die(mysqli_error($cnx) . "Error: en consultar el inventario 3");
