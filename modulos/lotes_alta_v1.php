  				...
  				<div class="row">   
					 ...
					 //$cad_est = mysqli_query($cnx,"select * from listado_estatus WHERE le_aplica = 'P' order by le_estatus asc");
  								...
  								while($reg_est =  mysqli_fetch_assoc($cad_est)) {
  									...
  									if($reg_est['pp_id'] == 1){$str_es = '1A';}else{$str_es = '1B';}
  									?>
  									<option value="<?php echo mb_convert_encoding($reg_est['prop_id'], "UTF-8");  ?>" 
  										<?php if(mb_convert_encoding($reg_est['prop_id'], "UTF-8") == $registros['le_id']){ ?>selected="selected"<?php }?>><?php echo mb_convert_encoding($reg_est['prop_id'], "UTF-8").' / '.$str_es;  ?>
									</option>
  									<?php }?>
  								</select>
  							</div>
