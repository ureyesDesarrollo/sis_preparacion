...
{*/
	...
	$txt_mes = date("m");				
	 $cadena = mysqli_query($cnx, "INSERT INTO lotes(lote_fecha, lote_hora, lote_mes, lote_folio, lote_turno, usu_id) 
	...
	if ($reg_ultimo_id != '') {
		mysqli_query($cnx, "INSERT INTO lotes_procesos(lote_id, prop_id) 
						VALUES ('$reg_ultimo_id[res]', '$cbxPaleto') ") or die(mysqli_error($cnx)." Error al insertar el lote procesos");
	}
		
	//}while($reg_proc =  mysqli_fetch_assoc($cad_proc));
	
	$str_mensaje = "y Lote creado";
/*}	
...
ins_bit_acciones($_SESSION['idUsu'],'A', $reg_ultimo_id['res'], '18');

$respuesta = array('mensaje' => "Lote Creado ");
