 <script>
   function filtro() {
     ...
     if (fechaIni != '' && fechaFin != '') {
       $.ajax({
         type: 'post',
         url: 'inventario_filtro_rango.php',
         data: datos,
         //data: {nombre:n},
         success: function(d) {
           $("#tab2").html(d);
   ...
   function exportar() {
     ...
     var fechaFin = document.getElementById('fechaFinal').value;
     if (fechaIni != '' && fechaFin != '') {
       window.open('../exportar/inventario_exportar_rango.php?fechaIni=' + encodeURIComponent(fechaIni) +
     ...
     if (fechaIni == '' && fechaFin == '') {
       window.open('../exportar/inventario_exportar_dia.php');
     }
   ...
   function reset() {
     location.href = "listado_inventario.php";
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fecha >= '" . date("Y-m-d") . "' AND prv_tipo = 'L'") or die(mysqli_error($cnx) . "Error: en consultar el inventario 2");
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE substring(inv_fe_recibe,1,10) = '" . date("Y-m-d") . "' and i.inv_enviado = 2") or die(mysqli_error($cnx) . "Error: en consultar el inventario 2");
  $registros3 = mysqli_fetch_assoc($cadena3);

  /*$cad_scat = mysqli_query($cnx, "SELECT DISTINCT i.mat_id, m.mat_nombre
  FROM materiales AS m
  INNER JOIN inventario AS i ON ( m.mat_id = i.mat_id ) WHERE inv_fecha >= '".date("Y-m-d")."' "); 
   ...
   <div class="container">
    ...
    <table style="width: 1200px">
      <tr style="font-weight: bold;font-size: 20px;text-align: right;">
        ...
        <td width="250" >Total de entrada a planta</td>
        <td width="95" ><?php echo $flt_kg_inv + $flt_kg_inv2; ?></td>
        <td width="90" style="border-right: 1px solid#fff;border-bottom: 1px solid#fff;border-top: 1px solid#fff"></td>
         ...
         <table style="width: 1200px;">
           <tr style="background: #32383e;color: #fff">
             ...
             <td width="59%" rowspan="5" align="center" valign="top" style="background: #fff;border-right: 1px solid#fff;border-bottom: 1px solid#fff;border-top: 1px solid#fff">
