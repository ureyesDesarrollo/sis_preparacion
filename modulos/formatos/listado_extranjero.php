...
$cadena = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre,prv_ncorto
 FROM inventario as i
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE p.prv_tipo = 'E' and inv_enviado = 0 and inv_tomado = 0 ") or die(mysqli_error($cnx) . "Error: en consultar el inventario 1");
        ...
        <table style="border: 1px solid #fff">
          <tr style="border: 1px solid #fff">
            ...
            <td style="border: 1px solid #fff">
              <h1>Inventario extranjero al día <?php echo fnc_formato_fecha(date("Y-m-d")); ?></h1>
            </td>
          </tr>
          <tr></tr>
        </table>
        ...
        <tbody>
          ...
          do { ?>
            <tr style="border: 1px solid">
              ...
              <!-- <td><?php $str_cadena = $registros['inv_id'] . strtoupper($registros['prv_nombre']);
              echo xtransforma($str_cadena, 6);
              echo transformar($registros['prv_nombre']);?></td> -->
