...
<?php
do {
    $cad_material = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_ncorto,p.prv_tipo, m.mat_nombre
...
?>
        <table class="table table-bordered" id="tabla_inventario">
            <thead>
                <tr>
                    <th colspan="16" style="background: #fff;color: #000;text-align: center;text-transform:uppercase "><?php echo $reg_tipo_material['mt_descripcion']; ?></th>
                </tr>
                <tr>
            ...
            <tbody>
                ...
                ?>
                        <tr style="<?php echo $color . ''; ?>">
                            ...
                            <td align="right"><?php echo $kilos_totales; ?></td>
                    ...
                    <?php
                        ...
                        $total_inv_ext += $reg_material['inv_kilos'];
                    } else { ?>
                        <tr>
                ...
                <tr>
                    <td colspan="9"></td>
                    <td style="text-align: right;font-weight: bold;font-size: 18px;"><?php echo number_format($total_inv_ext); ?></td>
                    <td colspan="4"></td>
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        do {
            if ($tot_dep_maq > 0) {
                ...
                if ($reg_dep_maq['inv_id'] != '') {
                    ...
                    $cad_inv3 = mysqli_query($cnx, "SELECT inv_kg_totales FROM `inventario` WHERE inv_id = " . $reg_dep_maq['inv_id'] . " OR 
					inv_id_key = " . $reg_dep_maq['inv_id'] . "") or die(mysqli_error($cnx));
                    ...
                    //suma las parcialidades con el mismo numero de inventario que el original 

                    //$kg_pagar3 = $reg_inv3['inv_kg_totales'] + $reg_sum3['inv_kg_totales'];
                    $kg_pagar3 = $reg_inv3['inv_kg_totales'];
                }
        ?>
                <tr>
                    ...
                    <td><?php echo $reg_inv3['inv_kg_totales'] ?></td>
                </tr>
            <?php
                $ren += 1;
                $flt_kg_inv3 += $kg_pagar3;
                $total_inv_kilos += $reg_dep_maq['inv_kilos'];
            } else { ?>
        ...
        <tr>
            ...
            <td style="text-align: right;font-weight: bold;font-size: 18px;"><?php echo number_format($total_inv_kilos); ?></td>
            <td colspan="4"></td>
...
$cadena_ext_bodega = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_ncorto,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE $filtro_fecha AND prv_tipo = 'E' AND inv_id_key is NULL AND inv_enviado = 0 ") or die(mysqli_error($cnx) . "Error: en consultar el inventario 1");
...
<table class="table table-bordered">
    <thead>
        <tr>
            ...
            <th colspan="14" style="background: #fff;color: #000;text-align: center;">1 - COMPRA DE PEDACERA AMERICANA (CAMIONES A BODEGA)</th>
        </tr>1 - COMPRA DE PEDACERA AMERICANA (CAMIONES A BODEGA)
        <tr>
            <th>No. Ticket</th>
            <th>No. viaje/consec</th>
            <th>Entrada</th>
    ...
    <tbody>
        ...
        ?>
                <tr>
                    ...
                    <td><?php echo $reg_ext_bodega['inv_prueba_rendimiento'] ?></td>
                </tr>
            <?php
                $ren += 1;
...
INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
WHERE $inv_fe_enviado AND prv_tipo = 'E' AND inv_enviado in(1,5,6)") or die(mysqli_error($cnx) . "Error: en consultar el inventario 1");
...
if($reg_ext_maquila['inv_enviado'] == 6){ $kilos = $reg_ext_maquila['inv_kg_totales']; }else{ $kilos = $reg_ext_maquila['inv_kilos']; }
        ?>
                <tr>
                    ...
                    <td align="right">
					<!--<?php echo $reg_ext_maquila['inv_kilos'] ?>-->
					<?php echo $kilos;  ?></td>
...
<table class="table" margin-bottom: 100px;border: 0px">
