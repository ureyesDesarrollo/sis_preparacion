...
$cadena = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 WHERE inv_fecha = '$filtro' AND prv_tipo = 'E'") or die(mysql_error()."Error: en consultar el inventario 1");
...
$cadena2 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fecha = '$filtro' AND prv_tipo = 'L'") or die(mysql_error()."Error: en consultar el inventario 2");
...
$cadena3 = mysqli_query($cnx, "SELECT i.*, p.prv_nombre,p.prv_tipo, m.mat_nombre
 ...
 INNER JOIN proveedores AS p ON (i.prv_id = p.prv_id)
 INNER JOIN materiales AS m ON (i.mat_id = m.mat_id)
 WHERE inv_fe_recibe = '$filtro' and i.inv_enviado = 2") or die(mysql_error()."Error: en consultar el inventario 2");
      ...
      <div class="tablehead" style="width: 1200px">
       <table width="1187" style="border: 1px solid #fff">
        <tr style="border: 1px solid #fff">
         ...
         <td align="center" style="border: 1px solid #fff"><h1>ENTRADAS DE MATERIA PRIMA</h1></td>
         <td align="center" style="border: 1px solid #fff"><p>REF F 001-REV.000</p>
           <p style="font-size: 20px">FECHA : <?php echo $filtro; ?></p></td>
         </tr>
       </table>
     </div>
     ...
     <tbody>
      ...
      do{?>
        <tr style="border: 1px solid">
          ...
          <td><?php echo $registros3['inv_kg_totales']?></td>
            </tr>
            <?php 
            $ren += 1;
          ...
          <tr>
            <td colspan="5" style="border-left: 1px solid#fff;border-bottom: 1px solid#fff;border-right: 1px solid#fff"></td>
            <td style="text-align: right;font-weight: bold;font-size: 18px;border-bottom: 1px solid#fff;">Total</td>
      ...
      <table style="width: 1200px">
        <tr style="font-weight: bold;font-size: 20px;text-align: right;">
          ...
          <td width="95" ><?php echo $flt_kg_inv + $flt_kg_inv2 + $flt_kg_inv3; ?></td>
        ...
        <tr style="background: #32383e;color: #fff">
	  	...
	  	//Obtiene el objetivo por tipo de material
              $inv_obj = mysqli_query($cnx, "SELECT mto_kilos as res
                ...
                WHERE mt_id = '$reg_scat[mt_id]' and mto_fecha = '$filtro'
              ...
              $inv = mysqli_query($cnx, "SELECT sum(i.inv_kg_totales)as res2 
                FROM inventario as i 
                INNER JOIN materiales as m  ON(i.mat_id = m.mat_id)
                WHERE m.mt_id = '$reg_scat[mt_id]' and inv_fecha = '$filtro'
                ") or die(mysql_error()."Error: en consultar el inventario 3");
