...
$cadena = mysqli_query($cnx, "SELECT p.*, l.le_estatus
 								FROM preparacion_lavadores as p
								INNER JOIN listado_estatus as l on (p.le_id = l.le_id)") or die(mysqli_error($cnx)."Error: en consultar los lavadores");
  ...
  <tbody>
   ...
   do{?>
    <tr height="20">
     ...
     <td><?php echo $registros['pl_descripcion'] ?></td>
	 <td><?php echo mb_convert_encoding($registros['le_estatus'],"UTF-8") ?></td>
     <td style="padding-left: 0px" align="center"><?php if(fnc_permiso($_SESSION['privilegio'], 13, 'upe_editar' ) == 1){?><a href="#" onClick="javascript:fnc_abre_modal(<?=$registros['pl_id']; ?>)"><img src="../iconos/editar.png"></a><?php }?></td>
   </tr>
   <?php 
   $ren += 1;
