...
$sql_eq_g = mysqli_query($cnx, "SELECT ep_descripcion FROM  equipos_preparacion  WHERE ep_id = '$id_e'") or die(mysqli_error($cnx) . "Error: en consultar el proceso");
$reg_eq_g = mysqli_fetch_assoc($sql_eq_g);
        ...
        <table class="table table-bordered">
          ...
          do {
            ...
            do {
              if (isset($reg_procesos_a['pro_id'])) {
                $sql_procesos_eq = mysqli_query($cnx, "SELECT  eq.ep_descripcion,eq.ep_carga_max
                FROM procesos_equipos as p
                      inner join equipos_preparacion as eq on (p.ep_id = eq.ep_id)
                      WHERE p.pro_id = '" . $reg_procesos_a['pro_id'] . "' 
            ...
            inner join preparacion_etapas as e on(t.pe_id = e.pe_id) where pt_id = '$reg_g_procesos[pt_id]' ") or die(mysqli_error($cnx) . "Error: en consultar el proceso");
                ...
                /* $sqlE = mysqli_query($cnx, "SELECT x.ep_descripcion, x.ep_carga_min, x.ep_carga_max
                      ...
                      WHERE e.pro_id = '$reg_g_procesos[pro_id]'
                      AND pe_ban_activo = '1'") or die(mysqli_error($cnx) . "Error: en consultar el proceso"); */
                $sqlE = mysqli_query($cnx, "SELECT x.ep_descripcion, x.ep_carga_min, x.ep_carga_max
                      FROM equipos_preparacion AS x
                      INNER JOIN procesos_equipos AS e ON x.ep_id = e.ep_id
          ...
          ?>
                <tr>
                  ...
                  <td><?php if (isset($_GET['idx_pro'])) {
                        echo $regE['ep_descripcion'];
                      } else {
                        echo $reg_eq_g['ep_descripcion'];
                      } ?></td>
                  <td style="text-align: right;"><?php echo $reg_procesos_eq['ep_carga_max']; ?></td>
