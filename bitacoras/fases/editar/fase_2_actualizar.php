...
mysqli_query($cnx, "UPDATE procesos_fase_2_g SET pfg2_temp_ag = '0', pfg2_ph_ant = '$txtPhAnt', pfg2_ce = '$txtCe', pfg2_sosa = '$txtAjSosa', pfg2_ph_aju = '$txtPhAj', pfg2_peroxido = '$txtPeroxido' WHERE pro_id = '$hdd_pro_id' AND pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx) . " Error al actualizar 2");
...
for ($i = 1; $i <= 10; $i++) {
	...
	if ($txtHr != '' and $txtPh != '' and $txtRedox != '') {
		...
		mysqli_query($cnx, "UPDATE procesos_fase_2_d SET pfd2_hr = '$txtHr', pfd2_ph = '$txtPh', pfd2_sosa = '$txtSosa', pfd2_peroxido='0', pfd2_temp = '$txtTemp', pfd2_redox = '$txtRedox', pfd2_acido = '0' WHERE pfg2_id = '$hdd_pfg' AND pfd2_ren = '$txtRen' ") or die(mysqli_error($cnx) . " Error al actualizar el renglon " . $i);
	}
}
