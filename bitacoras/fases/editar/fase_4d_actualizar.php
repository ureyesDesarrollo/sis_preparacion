...
mysqli_query($cnx, "UPDATE procesos_fase_4b_g SET pfg4_horas_reales = '$txtHrsReales' WHERE  pro_id = '$hdd_pro_id' AND pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx) . " Error al actualizar 2");
...
for ($i = 1; $i <= 10; $i++) {
	...
	if ($cbxTipAg != '' or $txtTemp != '' or $txtHraIni != '' or $txtHraFin != '' or $txtHraIniMov != '' or $txtHraFinMov != '' or $txtPh != '' or $txtCe != '' or $txtagua_a != '') {
		...
		if ($txtTemp != '') {
			$str_update .= ", pfd4_temp = '$txtTemp'";
		}
		if ($txtHraIni != '') {
			$str_update .= ", pfd4_hr_ini = '$txtHraIni'";
		}
		if ($txtHraFin != '') {
			$str_update .= ", pfd4_hr_fin = '$txtHraFin'";
		}
		if ($txtHraIniMov != '') {
			$str_update .= ", pfd4_hr_ini_mov = '$txtHraIniMov'";
		}
		if ($txtHraFinMov != '') {
			$str_update .= ", pfd4_hr_fin_mov = '$txtHraFinMov'";
		}
		if ($txtPh != '') {
			$str_update .= ", pfd4_ph = '$txtPh'";
		}
		if ($txtCe != '') {
			$str_update .= ", pfd4_ce = '$txtCe'";
		}
		if ($txtPpm != '') {
			$str_update .= ", pfd4_ppm = '$txtPpm'";
		}
		if ($txtagua_a != '') {
			$str_update .= ",taa_id = '$txtagua_a'";
		}
		if ($txtObs != '') {
			$str_update .= ", pfd4_observaciones = '$txtObs' ";
		}
