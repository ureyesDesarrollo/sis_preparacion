...
for ($i = 1; $i <= 10; $i++) {
	...
	if ($txtMov != '' or $txtHIniDrenado != '' or $txtHFinDrenado != '' or $txtPh != ''  or $txtCe != '' or $txtTemp != '' or  $txtObs != '') { # $cbxTipAgd != '' or $txtagua_a != '' or

		//if($cbxTipAgd != ''){$str_update .= ",tpa_id = '$cbxTipAgd'";}
		if ($txtMov != '') {
			$str_update .= ", pfd7_mov = '$txtMov'";
		}
		if ($txtHIniDrenado != '') {
			$str_update .= ", pfd7_hr_ini_dren = '$txtHIniDrenado'";
		}
		if ($txtHFinDrenado != '') {
			$str_update .= ", pfd7_hr_fin_dren = '$txtHFinDrenado'";
		}
		if ($txtPh != '') {
			$str_update .= ", pfd7_ph = '$txtPh'";
		}
		if ($txtCe != '') {
			$str_update .= ", pfd7_ce = '$txtCe'";
		}
		if ($txtTemp != '') {
			$str_update .= ", pfd7_temp = '$txtTemp'";
		}
		//if($txtagua_a != ''){$str_update .= ",taa_id = '$txtagua_a'";}
		if ($txtObs != '') {
			$str_update .= ", pfd7_observaciones = '$txtObs' ";
		}
