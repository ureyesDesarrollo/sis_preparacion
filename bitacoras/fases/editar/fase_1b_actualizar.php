...
mysqli_query($cnx, "UPDATE procesos_fase_1_g SET pfg1_ph_agua = '$txtAgIni', pfg1_ce_agua = '$txtCeIni' WHERE pro_id = '$hdd_pro_id' AND pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx)." Error al actualizar 2");
...
for($i = 1; $i <= 10; $i++)
{
	...
	if($cbxTipAg != '' or $txtTemp != '' or $txtHraIni != '' or $txtHraFin != '' or  $txtHraIniMov != '' or $txtHraFinMov != '' or $txtPh != '' or $txtCe != '' or $txtagua_a != '')
	{
		...
		if($txtTemp != ''){$str_update .= ", pfd1_temp = '$txtTemp'";}
		if($txtHraIni != ''){$str_update .= ", pfd1_hr_ini = '$txtHraIni'";}
		if($txtHraFin != ''){$str_update .= ", pfd1_hr_fin = '$txtHraFin'";}
		if($txtHraIniMov != ''){$str_update .= ", pfd1_hr_ini_mov = '$txtHraIniMov'";}
		if($txtHraFinMov != ''){$str_update .= ", pfd1_hr_fin_mov = '$txtHraFinMov'";}
		if($txtPh != ''){$str_update .= ", pfd1_ph = '$txtPh'";}
		if($txtCe != ''){$str_update .= ", pfd1_ce = '$txtCe'";}
		if($txtagua_a != ''){$str_update .= ",taa_id = '$txtagua_a'";}
		if($txtObs != ''){$str_update .= ", pfd1_observaciones = '$txtObs' ";}

		/*mysqli_query($cnx, "UPDATE procesos_fase_1_d SET tpa_id = '$cbxTipAg', pfd1_temp = '$txtTemp', pfd1_hr_ini = '$txtHraIni', pfd1_hr_fin = '$txtHraFin', pfd1_hr_ini_mov = '$txtHraIniMov', pfd1_hr_fin_mov = '$txtHraFinMov', pfd1_ph = '$txtPh', pfd1_ce = '$txtCe',taa_id = '$txtagua_a' ,pfd1_observaciones = '$txtObs' WHERE pfg1_id = '$hdd_pfg' AND pfd1_ren = '$txtRen' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);*/
