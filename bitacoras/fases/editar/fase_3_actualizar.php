...
if ($txtFeTerm == '' and $txtHrTerm == '') {
	...
	//mysqli_query($cnx, "UPDATE procesos_fase_3_g SET pro_id = '$hdd_pro_id', pfg3_enzima = '$txtEnzima', usu_id = '" . $_SESSION['idUsu'] . "' WHERE pro_id = '$hdd_pro_id'") or die(mysqli_error($cnx) . " Error al insertar 2");
}
...
for ($i = 1; $i <= 10; $i++) {
	...
	if ($cbxTipAg != '' or $txtTemp != '' or $txtHraIni != '' or $txtHraFin != '' or $txtHraIniMov != '' or $txtHraFinMov != '' or $txtPh != '' or $txtCe != '' or $txtPpm != '' ) {

		if ($cbxTipAg != '') {
			$str_update .= ",tpa_id = '$cbxTipAg'";
		}
		if ($txtTemp != '') {
			$str_update .= ", pfd3_temp = '$txtTemp'";
		}
		if ($txtHraIni != '') {
			$str_update .= ", pfd3_hr_ini = '$txtHraIni'";
		}
		if ($txtHraFin != '') {
			$str_update .= ", pfd3_hr_fin = '$txtHraFin'";
		}
		if ($txtHraIniMov != '') {
			$str_update .= ", pfd3_hr_ini_mov = '$txtHraIniMov'";
		}
		if ($txtHraFinMov != '') {
			$str_update .= ", pfd3_hr_fin_mov = '$txtHraFinMov'";
		}
		if ($txtPh != '') {
			$str_update .= ", pfd3_ph = '$txtPh'";
		}
		if ($txtCe != '') {
			$str_update .= ", pfd3_ce = '$txtCe'";
		}
		if ($txtPpm != '') {
			$str_update .= ", pfd3_ppm = '$txtPpm'";
		}
		/*if ($txtagua_a != '') {
			$str_update .= ",taa_id = '$txtagua_a'";
		}*/
		if ($txtObs != '') {
			$str_update .= ", pfd3_observaciones = '$txtObs' ";
		}
