...
mysqli_query($cnx, "UPDATE procesos_fase_4b_g SET pfg4_temp_ag = '$txtTemp', pfg4_ph = '$txt_ph_d', pfg4_ce = '$txt_ce_d' WHERE pro_id = '$hdd_pro_id' AND pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx)." Error al actualizar 2");
...
/*for($i = 1; $i <= 10; $i++)
{
	...
	{
		mysqli_query($cnx, "UPDATE procesos_fase_1_d SET tpa_id = '$cbxTipAg', pfd1_temp = '$txtTemp', pfd1_hr_ini = '$txtHraIni', pfd1_hr_fin = '$txtHraFin', pfd1_hr_ini_mov = '$txtHraIniMov', pfd1_hr_fin_mov = '$txtHraFinMov', pfd1_ph = '$txtPh', pfd1_ce = '$txtCe', pfd1_observaciones = '$txtObs' WHERE pfg1_id = '$hdd_pfg' AND pfd1_ren = '$txtRen' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);
...
for($i = 1; $i <= 10; $i++)
{
	...
	if($cbxTipAg != '' or $txtTemp != '' or $txtHraIni != '' or $txtHraFin != '' or $txtHraIniMov != '' or $txtHraFinMov != '' or $txtPh != '' or $txtCe != '' or $txtagua_a != '')
	{
		...
		/*mysqli_query($cnx, "UPDATE procesos_fase_4b_d SET tpa_id = '$cbxTipAg', pfd4_temp = '$txtTemp', pfd4_hr_ini = '$txtHraIni', pfd4_hr_fin = '$txtHraFin', pfd4_hr_ini_mov = '$txtHraIniMov', pfd4_hr_fin_mov = '$txtHraFinMov', pfd4_ph = '$txtPh', pfd4_ce = '$txtCe',taa_id = '$txtagua_a' , pfd4_observaciones = '$txtObs' WHERE pfg4_id = '$hdd_pfg' AND pfd4_ren = '$txtRen' ") or die(mysqli_error($cnx)." Error al actualizar el renglon ".$i);*/
