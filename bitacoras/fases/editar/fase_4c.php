...
$cad_aux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 9");
$reg_aux = mysqli_fetch_array($cad_aux);

$cad_fa = mysqli_query($cnx, "SELECT * FROM procesos_fase_4b_g WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 9");
$reg_fa = mysqli_fetch_array($cad_fa);

$cad_lib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 9");
...
/*	a = 6;
	...
	*/
	...
	{
		$("#formFase4cE").submit(function(){

			var formData = $(this).serialize();
			$.ajax({
				url: "fases/editar/fase_4c_actualizar.php",
				type: 'POST',
				data: formData,
				success: function(result) {

					data = JSON.parse(result);
					alertas("#alerta-errorFase4cEOpe", 'Listo!', data["mensaje"], 1, true, 5000);
					$('#formFase4cE').each (function(){this.reset();});  
	...
	var datos={
		...
		$.ajax({
			...
			//data: {nombre:n},
			success: function(result){
				$("#modalRenglon4c").html(result);
				$('#modalRenglon4c').modal('show')
			}	
		});
		return false;
	}
	...
	var datos={
		...
		$.ajax({
			...
			//data: {nombre:n},
			success: function(result){
				$("#modalPaleto4c").html(result);
				$('#modalPaleto4c').modal('show')
			}	
		});
		return false;
	}
	...
	//Bloquear boton guardar
		function confirmEnviar5() {
			...
			formFase4cE.btn.disabled = true; 
			formFase4cE.btn.value = "Enviando...";

			setTimeout(function(){
				formFase4cE.btn.disabled = true;
				formFase4cE.btn.value = "Guardar";
			},2000);
...
<form autocomplete="off" id="formFase4cE" name="formFase4cE">
	<input name="hdd_pro_id" type="hidden" value="<?php echo $reg_pro['pro_id'] ?>" id="hdd_pro_id"/>
	<input name="hdd_pe_id" type="hidden" value="9" id="hdd_pe_id"/>
	...
	<div class="row" style="margin-bottom: 10px">
		...
		<div class="col-md-2">
				...
				<input style="width: 100px;display:inline" type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="txt_ph_d" name="txt_ph_d" placeholder="PH inicial" value="<?php if ($reg_fa['pfg4_ph'] == '') {
																																																								echo "";
																																																								$str_prop = 'disabled';
																																																							} else {
																																																								echo $reg_fa['pfg4_ph'];
			...
			<div class="col-md-2">
				<label>CE inicial</label>
				<input style="width: 100px;display:inline" type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="txt_ce_d" name="txt_ce_d" placeholder="CE inicial" value="<?php if ($reg_fa['pfg4_ce'] == '') {
																																																								echo "";
																																																								$str_prop = 'disabled';
																																																							} else {
																																																								echo $reg_fa['pfg4_ce'];
		...
		<label class="col-md-1" style="width: 165px">Temp inicial</label>
		<div class="col-md-1 tiempos">
			<input type="text" maxlength="6" onKeyPress="return isNumberKey(event, this);" class="form-control" id="txtTemp" name="txtTemp" placeholder="TEMP" value="<?php if($reg_fa['pfg4_temp_ag'] == ''){echo ""; $str_prop = 'disabled';}else{echo $reg_fa['pfg4_temp_ag']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3; ?> required>
		</div>
...
$fltVal = fnc_parametro_max(9, 'C');

$bolVal = fnc_valida_renglon($i, $reg_pro['pro_id'], 9);
...
if($i <= 3 or $bolVal == 'Si' ){
  ...
  <tr>
   <td>&nbsp;</td>
    ...
    <td><input type="hidden" class="form-control" id="<?php echo "txtRen".$i ?>" value="<?php echo $i ?>" name="<?php echo "txtRen".$i ?>">
							<!--<input type="text" maxlength="6" onKeyPress="return isNumberKey(event, this);" class="form-control" id="<?php /*echo "txtLavTipAgua".$i ?>" name="<?php echo "txtLavTipAgua".$i ?>"  value="<?php if($reg_fad['pfd4_tipo_ag'] == ''){echo ""; $str_prop = '';}else{echo $reg_fad['pfd4_tipo_ag']; $str_prop = 'disabled';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3; echo $strProp4;*/ ?> size="5">-->
    ...
    <!--<td><input type="text" maxlength="6" onKeyPress="return isNumberKey(event, this);" class="form-control" id="<?php /*echo "txtTemp".$i ?>" name="<?php echo "txtTemp".$i ?>" value="<?php if($reg_fad['pfd4_temp'] == ''){echo ""; $str_prop = 'disabled';}else{echo $reg_fad['pfd4_temp']; $str_prop = '';} ?>" <?php echo $str_prop;  echo " ".$strProp2; echo " ".$strProp3; echo " ".$strProp4; */?> size="5" placeholder="Temp"></td>-->
	...
	<td><input type="text" maxlength="6" onKeyPress="return isNumberKey(event, this);" class="form-control" id="<?php echo "txtCe".$i ?>" name="<?php echo "txtCe".$i ?>" value="<?php if($reg_fad['pfd4_ce'] == ''){echo ""; $str_prop = 'disabled';}else{echo $reg_fad['pfd4_ce']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3;  echo " ".$strProp4;?> size="5" placeholder="Ce" <?php if($fltVal >= $reg_fad['pfd4_ce'] and $reg_fad['pfd4_ce'] != ''){?>style="background-color:#66FF66;"<?php }?>></td>
    <td><input type="text" maxlength="6" onKeyPress="return isNumberKey(event, this);" class="form-control" id="<?php echo "txtPh".$i ?>" name="<?php echo "txtPh".$i ?>" value="<?php if($reg_fad['pfd4_ph'] == ''){echo ""; $str_prop = 'disabled';}else{echo $reg_fad['pfd4_ph']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3;  echo " ".$strProp4;?> size="5" placeholder="pH"></td>
    <!--<td align="center">
  ...
  <td><input type="text" class="form-control" maxlength="100" id="<?php echo "txtObs".$i ?>" name="<?php echo "txtObs".$i ?>" value="<?php if($reg_fad['pfd4_observaciones'] == ''){echo ""; $str_prop = 'disabled';}else{echo $reg_fad['pfd4_observaciones']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp2; echo " ".$strProp3;  echo " ".$strProp4;?> size="5" placeholder="Observaciones"></td>
  </tr>
  <?php 
  $valUltRen = $i;
  }//termina if
...
<!--	<div class="row">
		<div class="form-row">
			<div class="form-group col-md-1" style="width: 30px">
				...
				<label for="inputPassword3" style="margin-bottom: 10px">1er</label>
				<label for="inputPassword3" style="margin-bottom: 10px">2do</label>
				<label for="inputPassword3" style="margin-bottom: 10px">3er</label>
				<label for="inputPassword3" style="margin-bottom: 10px">4to</label>
			</div>
		</div>
		<div class="form-row">
		...
		<div class="col-md-3 divEtapas">
			...
			<div class="col-md-2 divEtapasInput">
				...
				<input  type="text" id="txtCeLib" class="form-control" placeholder="Ce liberacion" name="txtCeLib" value="<?php if($reg_lib['prol_ce'] == ''){ echo ""; $str_prop = 'disabled';}else{echo $reg_lib['prol_ce']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp1; echo " ".$strProp2;?>>
					<input  maxlength="6" onKeyPress="return isNumberKey(event, this);" type="text" id="txtHrTotales" class="form-control" placeholder="Horas totales" name="txtHrTotales" value="<?php if($reg_lib['prol_hr_totales'] == ''){ echo ""; $str_prop = 'disabled';}else{echo $reg_lib['prol_hr_totales']; $str_prop = '';} ?>" <?php echo $str_prop; echo " ".$strProp1; echo " ".$strProp2;?>>
	...
	<div class="row" style="margin-bottom: 10px">
		<label style="font-weight:bold; margin-left:20px;">El agua de este proceso se manda a Pila 2 agua recuperada semilimpia</label>
	</div>
...
<!--barra botones-->
	<div class="row footerdivProcesos" style="margin-bottom: 10px;">
		...
		<div class="form-group col-md-7">
			<div class="alert alert-info hide" id="alerta-errorFase4cEOpe" style="height: 40px;width: 270px;text-align: left;z-index: 10;font-size: 10px;margin-bottom: -10px">
		...
		<?php /*if($_SESSION['privilegio'] == 4){	?>
				...
				<div class="col-md-2" style="width: 110px">
					<button type="button" class="btn btn-success" id="permitir" onClick="javascript:AbreModalAgregarR(<?php echo $reg_pro['pro_id'] ?>, 9, <?php echo $valUltRen ?>);"> <img src="../../iconos/add.png" alt="">Renglon
					</button>
				</div>
			<?php } */?>
...
<div class="modal" id="modalRenglon4c" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false"></div>

<div class="modal" id="modalPaleto4c" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false"></div>
