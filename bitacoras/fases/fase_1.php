...
<script>
	...
	function AbreModalE1(proceso, etapa) {
		var datos = {
			"pro_id": proceso,
			"pe_id": etapa
		}
		$.ajax({
			type: 'post',
			url: 'fases/editar/fase_1.php',
			data: datos,
			success: function(result) {
				$("#modalEditar1").html(result);
	...
	function quimicos_1(proceso, etapa) {
		var datos = {
			"pro_id": proceso,
			"pe_id": etapa
		}
...
<div class="divProcesos">
	<form autocomplete="off" id="formFase1" name="formFase1">
					...
					?>
							<tr>
								...
								<td><input type="hidden" class="form-control" id="<?php echo "txtRen" . $i ?>" value="<?php echo $i ?>" name="<?php echo "txtRen" . $i ?>">
									<input type="hidden" class="form-control" id="<?php echo "hddRen" . $i ?>" name="<?php echo "hddRen" . $i ?>" value="<?php echo $reg_fad1['pfd1_id'];/*if($reg_fad1['pfd1_id'] == ''){echo ""; $str_prop = '';}else{echo $reg_fad1['pfd1_id']; $str_prop = 'd';} ?>" <?php echo $str_prop;  echo " ".$strProp2; echo " ".$strProp3; echo " ".$strProp4;*/ ?>">
					...
					<div class="col-md-2 divEtapasInput">
						...
						<input type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" id="txtHrTotales" class="form-control" placeholder="Horas totales" name="txtHrTotales" value="<?php if ($reg_lib['prol_hr_totales'] == '') {
																																																				...
																																																				$str_prop = 'disabled';
																																																					...
																																																					echo " " . $strProp1;
																																																					echo " " . $strProp2; ?>>
						<input type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" id="txt_extractivilidad" class="form-control" placeholder="Extractibilidad" name="txt_extractivilidad" value="<?php if ($reg_lib['extractibilidad'] == '') {
																																																				echo "";
																																																				$str_prop = '';
																																																			} else {
																																																				echo $reg_lib['extractibilidad'];
																																																				$str_prop = 'disabled';
																																																			} ?>" <?php echo $str_prop;
																																																					echo " " . $strProp1;
																																																					echo " " . $strProp2; ?>>
		...
		<div class="row">
			...
			<!-- <div class="form-group col-md-2">
				...
				<input type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="txt_extractivilidad" name="txt_extractivilidad" placeholder="Extractibilidad" value="<?php if ($reg_fa1['pfg1_extractivilidad'] == '' || $reg_fa1['pfg1_extractivilidad'] == 0) {
																																																						echo "";
																																																						$str_prop = '';
																																																					} else {
		...
		<div class="row" <?php echo $oculta_opciones ?> style="margin-right: 0px">
			<div class="col-md-8">
				<div class="alert alert-info hide" id="alerta-errorFase1Ope" style="height: 40px;width: 270px;text-align: left;z-index: 10;font-size: 10px;margin-bottom: -10px">
			...
			<div class="col-md-4" style="margin-top:1rem;margin-bottom:1rem;text-align:right">
				...
				<?php if ($_SESSION['privilegio'] == 4) {	?>
					...
					<button type="button" class="btn btn-success" id="editar" onClick="javascript:AbreModalE1(<?php echo $reg_pro['pro_id'] ?>, 1);"> <img src="../iconos/edit.png" alt="">Editar
					</button>
				...
				?>
					...
					<button type="button" class="btn btn-info" onClick="javascript:quimicos_1(<?php echo $reg_pro['pro_id'] ?>, <?php echo $reg_et['pe_id'] ?> );"><img src="../iconos/matraz.png" alt="">Químicos</button>
					<button type="submit" class="btn btn-primary" id="btn"><img src="../iconos/guardar.png" alt="">Guardar</button>
				<?php } ?>
			</div>
		</div>
	...
	<div class="modal" id="modalEditar1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false"></div>
...
<script>
	function requerido_1() {
		...
		if (fechaTerminacion.value !== '') {
			...
			document.getElementById('txtHrTerm').setAttribute('required', 'required');
			document.getElementById('txt_temp_final').setAttribute('required', 'required');
			document.getElementById('txt_extractivilidad').setAttribute('required', 'required');

		} else {
			// Si la fecha no está seleccionada, quita el atributo "required" del campo de horas totales
			document.getElementById('txtHrTerm').removeAttribute('required');
			document.getElementById('txt_temp_final').removeAttribute('required');
			document.getElementById('txt_extractivilidad').removeAttribute('required');
