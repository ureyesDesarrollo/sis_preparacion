...
else
{
	for($i = 1; $i <= 10; $i++)
	{
				...
				if($hddRen == '' and $txtHraIni != '')
		{
			...
			//mysqli_query($cnx, "INSERT INTO procesos_fase_5_d(pfg5_id, pfd5_ren, tpa_id,pfd5_hr_ini, usu_id, pfd5_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen','0', '$txtHraIni', '".$_SESSION['idUsu']."', SYSDATE() )") or die(mysqli_error($cnx)." Error al insertar renglon ".$i);
			mysqli_query($cnx, "INSERT INTO procesos_fase_5_d(pfg5_id, pfd5_ren, pfd5_hr_ini, usu_id, pfd5_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen', '$txtHraIni', '".$_SESSION['idUsu']."', SYSDATE() )") or die(mysqli_error($cnx)." Error al insertar renglon ".$i);
				$strMsj = "Se agrego el renglon ".$i;
		}
		else
		{ 
	...
	else
	{
		...
		{ 
			...
			/*mysqli_query($cnx, "UPDATE procesos_fase_4b_g SET pfg4_agua_a = '$txtAguaA', pfg4_horas_reales = '$txtHrsReales' WHERE pfg4_id = '$hdd_pfg' ") or die(mysqli_error($cnx)." Error al actualizar 4g");*/
