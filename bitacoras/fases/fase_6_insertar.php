...
if ($txtFeIni != '') {
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_6_g(pro_id,pe_id, pfg6_temp_ag, pfg6_temp, pfg6_acido, pfg6_temp2, pfg6_norm,pfg6_acido_diluido, pfg6_ph, usu_id) VALUES('$hdd_pro_id', '$hdd_pe_id', '$txtTemp', '$txtTemp2', '$txtAcido', '$txtTemp3', '$txtNorm', '$cbxDiluido', '$txtPh', '" . $_SESSION['idUsu'] . "' ) ") or die(mysqli_error($cnx) . " Error al insertar 2");
	...
	if ($strMsj != '') {
		...
		if ($txtFeTerm != '') {
			...
			if ($txtHrTotales != '') {
				mysqli_query($cnx, "INSERT INTO procesos_liberacion (usu_id, pro_id, pe_id, prol_hr_totales, prol_ph,prol_extrac2doacido) VALUES('" . $_SESSION['idUsu'] . "', '$hdd_pro_id', '$hdd_pe_id', '$txtHrTotales', '$txtPhLib','$txtExtr2doAcido') ") or die(mysqli_error($cnx) . " Error al insertar L");
