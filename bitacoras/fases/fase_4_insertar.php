...
if($txtFeIni != '')
{
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_4_g(pro_id, pe_id, pfg4_temp_ag, pfg4_temp, pfg4_acido, pfg4_termina, pfg4_temp2, pfg4_acido_fuerte,pfg4_enzima, usu_id) VALUES('$hdd_pro_id', '$hdd_pe_id', '$txtTemp', '$txtTemp2', '$txtAcido', '$txtTermina', '$txtTemp3', '$cbxAcidoF', '$txtEnzima', '".$_SESSION['idUsu']."' ) ") or die(mysqli_error($cnx)." Error al insertar 2");
	
	$respuesta = array('mensaje' => "Fase 4 agregada");
}
else
{
	for($i = 1; $i <= 8; $i++)
	{
		...
		{
			mysqli_query($cnx, "INSERT INTO procesos_fase_4_d(pfg4_id, pfd4_ren, pfd4_acido, pfd4_ph, pfd4_ph_b, pfd4_temp, pfd4_ppm, usu_id, pfd4_fe_hr_sys) VALUES('$hdd_pfg', '$txtRen', '$txtAcidoF', '$txtPhF', '$txtPhBF', '$txtTempF', '$txtPpm', '".$_SESSION['idUsu']."', SYSDATE() ) ") or die(mysqli_error($cnx)." Error al insertar ".$i);
	...
	else
	{
		...
		else
		{
			if($txtHrTotales != '' and $cbxAdel != '')
			{
				...
				$strMsj = "Fase 4 parametros capturados";
			}
			else
			{
				$strMsj = "Esta vacio";
