...
$cad_aux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 23");
$reg_aux = mysqli_fetch_array($cad_aux);

$cad_fa = mysqli_query($cnx, "SELECT * FROM procesos_fase_4b_g WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 23");
$reg_fa = mysqli_fetch_array($cad_fa);

$cad_lib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$reg_pro[pro_id]' and pe_id = 23");
...
<script>
	...
	//agregado 22-10-21
	function quimicos_4d(proceso, etapa) {
		var datos = {
			"pro_id": proceso,
			"pe_id": etapa
		}
	...
	$(document).ready(function() {
		$("#formFase4d").submit(function() {
			...
			$.ajax({
				url: "fases/fase_4d_insertar.php",
				type: 'POST',
				data: formData,
				success: function(result) {
...
<div class="divProcesos">
	...
	<form autocomplete="off" id="formFase4d" name="formFase4d">
		<input name="hdd_pro_id" type="hidden" value="<?php echo $reg_pro['pro_id'] ?>" id="hdd_pro_id" />
		<input name="hdd_pe_id" type="hidden" value="23" id="hdd_pe_id" />
			...
			?>
					<tr>
						...
						<td><input type="hidden" class="form-control" id="<?php echo "txtRen" . $i ?>" value="<?php echo $i ?>" name="<?php echo "txtRen" . $i ?>">
							...
							<select id="cbxTipAg<?php echo $i  ?>" class="form-control" style="width: 160px" name="cbxTipAg<?php echo $i  ?>" <?php if ($reg_fad['tpa_id'] == '') {
																																					echo "";
																																					$str_prop = '';
																																				} else {
																																					$str_prop = 'disabled';
						...
						<td><input type="text"  maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="<?php echo "txtTemp" . $i ?>" name="<?php echo "txtTemp" . $i ?>" value="<?php if ($reg_fad['pfd4_temp'] == '') {
																																																		echo "";
																																																		$str_prop = '';
																																																	} else {
						...
						<td><input type="text"  maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="<?php echo "txtCe" . $i ?>" name="<?php echo "txtCe" . $i ?>" value="<?php if ($reg_fad['pfd4_ce'] == '') {
																																																	...
																																																	$str_prop = 'disabled';
																																																		...
																																																		echo " " . $strProp2;
																																																		echo " " . $strProp3;
																																																		echo " " . $strProp4; ?> size="5" <?php if ($fltVal >= $reg_fad['pfd4_ce'] and $reg_fad['pfd4_ce'] != '') { ?>style="background-color:#66FF66;" <?php } ?> placeholder="Ce"></td>
						...
						<td align="center">
							<select id="cbxAgua<?php echo $i  ?>" class="form-control" style="width: 160px" name="cbxAgua<?php echo $i  ?>" <?php if ($reg_fad['taa_id'] == '') {
																																				echo "";
																																				$str_prop = '';
																																			} else {
																																				$str_prop = 'disabled';
		...
		<div class="row">
			...
			<div class="form-row">
				<div class="form-group col-md-2">
					...
					<input type="text"  maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="txtHrsReales" name="txtHrsReales" placeholder="Hrs reales" value="<?php if ($reg_fa['pfg4_horas_reales'] == '') {
																																															echo "";
																																															$str_prop = '';
																																														} else {
																																															echo $reg_fa['pfg4_horas_reales'];
																																															$str_prop = 'disabled';
																																														} ?>" <?php echo $str_prop;
																																																echo " " . $strProp6;
																																																echo " " . $strProp3 . " " . $strProp4; ?>>
		...
		<div class="row footerdivProcesos" style="margin-bottom: 10px;">
			...
			<div class="col-md-5" style="text-align: right;margin-right: -40px">
				...
				?>
					...
					<button type="button" class="btn btn-info" onClick="javascript:quimicos_4d(<?php echo $reg_pro['pro_id'] ?>,<?php echo $reg_et['pe_id'] ?> );"><img src="../iconos/matraz.png" alt="">Químicos</button>
					<button type="submit" class="btn btn-primary" id="btn"><img src="../iconos/guardar.png" alt="">Guardar</button>
				<?php } ?>
			</div>
