...
$sqlfg = mysqli_query($cnx, "SELECT * FROM procesos_fase_3b_g as pf3bg INNER JOIN procesos as p  on(pf3bg.pro_id=p.pro_id) WHERE p.pro_id  = '$idx_pro'") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_3b_g ");
$regfg = mysqli_fetch_assoc($sqlfg);


$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '6'") or die(mysqli_error($cnx) . "Error: en consultar procesos auxiliar");
$regProAux = mysqli_fetch_assoc($sqlProAux);


$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$regfg[pro_id]'  AND pe_id = '6'") or die(mysqli_error($cnx) . "Error: en consultar procesos de liberacion");
    ...
    <tr>
      <td colspan="2">
        <table border="1" style="background: #FCEFF2;font-size: 12px;width: 100%">
          <tr>
            <td height="45" colspan="10">ADICION A SOSA Nota:Estar revisando los chequeos durante las 32 horas</td>
          </tr>
        </table>
      </td>
    </tr>
    ...
    <tr>
      <td width="61%" valign="top">
        <table width="96%" border="1">
          ...
          <?php
          $sqlfd = mysqli_query($cnx, "SELECT * FROM procesos_fase_3b_d as pf3bd WHERE pf3bd.pfg3_id = '$regfg[pfg3_id]'") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_3b_d");
        ...
        <p>24 horas revisar el estado del material. Si ya esta "Liberar"</p>
      </td>
      <td rowspan="2">
        <table width="100%"  border="1">
          <tr>
    ...
    <tr>
      <td valign="top">
        ...
        <table border="1">
          ...
          <tr>
            <td><?php echo $regfg['pfg3_hr_totales'] ?></td>
            <td><?php echo fnc_nom_usu($regfg['usu_id']) ?></td>
            <td></td>
          </tr>
        </table>
