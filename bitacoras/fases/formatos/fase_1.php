...
$sqlf1g = mysqli_query($cnx, "SELECT * FROM procesos_fase_1_g as pf1g INNER JOIN procesos as p  on(pf1g.pro_id=p.pro_id) WHERE p.pro_id  = '$idx_pro' AND pf1g.pe_id = 1") or die(mysqli_error($cnx) . "Error: en consultar procesos_fase_1_g ");
...
$sqlProAux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$regf1g[pro_id]' AND pe_id = '1'") or die(mysqli_error($cnx) . "Error: en consultar procesos auxiliar");
...
$sqlProLib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$regf1g[pro_id]' AND pe_id = '1'") or die(mysqli_error($cnx) . "Error: en consultar procesos de liberacion");
      ...
      <td>
        <table border="1" width="100%">
          <tr style="font-weight: bold;">
            ...
            <td style="background: #e6e6e6">Temp final</td>
            <td style="background: #e6e6e6">Usuario</td>
            <td style="background: #e6e6e6;"><?php echo fnc_hora_de(1) ?> a <?php echo fnc_hora_a(1) ?> horas totales</td>
            <td rowspan="5" width="5%" style="font-size:50px; text-align: center;">1</td>
          ...
          <tr>
            ...
            <td colspan="2"><?php echo $regProLib['prol_ce'] ?></td>
          </tr>
          <tr>
            <td colspan="5"></td>
            <td>Extractibilidad</td>
            <td colspan="2"><?php echo $regProLib['extractibilidad'] ?></td>
          </tr>
          <tr>
            <td colspan="3" style="font-weight: bold;background: #e6e6e6">Observaciones:</td>
            <td colspan="2" style="border-bottom:1px solid#fff;border-top:1px solid#fff"></td>
            <td>Horas totales</td>
            <td colspan="3"><?php echo $regProLib['prol_hr_totales'] ?></td>
          </tr>
          <tr>
            <td colspan="3" ><?php echo $regProAux['proa_observaciones'] ?></td>
            <td colspan="2" style="border-bottom:1px solid#fff;border-top:1px solid#fff"></td>
            <td>Nombre LCP</td>
            <td colspan="3"><?php echo fnc_nom_usu($regProLib['usu_id']) ?></td>
          </tr>
        </table>
