...
$cad_aux = mysqli_query($cnx, "SELECT * FROM procesos_auxiliar WHERE pro_id = '$idx_pro' and pe_id = 9");
$reg_aux = mysqli_fetch_array($cad_aux);

$cad_fa = mysqli_query($cnx, "SELECT * FROM procesos_fase_4b_g WHERE pro_id = '$idx_pro' and pe_id = 9");
$reg_fa = mysqli_fetch_array($cad_fa);

$cad_lib = mysqli_query($cnx, "SELECT * FROM procesos_liberacion WHERE pro_id = '$idx_pro' and pe_id = 9");
...
<script>
	...
	$(document).ready(function() {
		$("#formFase4c").submit(function() {
			...
			$.ajax({
				url: "fases/fase_4c_insertar.php",
				type: 'POST',
				data: formData,
	...
	function AbreModalE4c(proceso, etapa) {
		var datos = {
			"pro_id": proceso,
			"pe_id": etapa
		}
		$.ajax({
			type: 'post',
			url: 'fases/editar/fase_4c.php',
			data: datos,
			success: function(result) {
				$("#modalEditar4c").html(result);
				$('#modalEditar4c').modal('show')
			}
		});
		return false;
	}
...
<div class="divProcesos">
	<form autocomplete="off" id="formFase4c" name="formFase4c">
		...
		<div class="row" style="margin-bottom: 10px">
			<div class="col-md-3">
				...
				<input style="width: 140px;display:inline" type="date" class="form-control" id="txtFeIni" placeholder="" name="txtFeIni" value="<?php if ($reg_aux['proa_fe_ini'] == '') {
																																					echo date("Y-m-d");
																																					$str_prop = '';
																																				} else {
			...
			<div class="col-md-3 ">
				...
				<input style="width: 140px;display:inline" type="time" class="form-control" id="txtHrIni" placeholder="" name="txtHrIni" value="<?php if ($reg_aux['proa_hr_ini'] == '') {
																																					echo date("H:i");
																																					$str_prop = '';
																																				} else {
			...
			<div class="col-md-2">
				...
				<input style="width: 80px;display:inline" type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="txtTemp" name="txtTemp" placeholder="Temp inicial" value="<?php if ($reg_fa['pfg4_temp_ag'] == '') {
																																																								echo "";
																																																								$str_prop = '';
																																																							} else {
			...
			<div class="col-md-2" style="height: 30px; border-radius: 5px; border:1px solid #e6e6e6; margin-bottom: 3px; font-weight:bold; background-color:#CCCCCC;width: 210px;">
				<label>BUSCAR CE 3.0 MAXIMO</label>
			</div>
		</div>
		<!--Renglones-->
		<table border="0" cellspacing="5" cellpadding="5">
			...
			?>
					<tr>
						...
						<td><input type="time" class="form-control" id="<?php echo "txtHraIni" . $i ?>" name="<?php echo "txtHraIni" . $i ?>" value="<?php if ($reg_fad['pfd4_hr_ini'] == '') {
																																							echo "";
																																							$str_prop = '';
																																						} else {
																																							echo $reg_fad['pfd4_hr_ini'];
																																							$str_prop = 'disabled';
																																						} ?>" <?php echo $str_prop;
																																								echo " " . $strProp2;
																																								echo " " . $strProp3;
																																								echo " " . $strProp4; ?>></td>
						<td><input type="time" class="form-control" id="<?php echo "txtHraFin" . $i ?>" name="<?php echo "txtHraFin" . $i ?>" value="<?php if ($reg_fad['pfd4_hr_fin'] == '') {
																																							echo "";
																																							$str_prop = '';
																																						} else {
																																							echo $reg_fad['pfd4_hr_fin'];
																																							$str_prop = 'disabled';
																																						} ?>" <?php echo $str_prop;
																																								echo " " . $strProp2;
																																								echo " " . $strProp3;
						...
						<td><input type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="<?php echo "txtPh" . $i ?>" name="<?php echo "txtPh" . $i ?>" value="<?php if ($reg_fad['pfd4_ph'] == '') {
																																																					echo "";
																																																					$str_prop = '';
																																																				} else {
						...
						<td><input type="text" maxlength="6" onKeyPress="return isNumberKeyFloat(event, this);" class="form-control" id="<?php echo "txtCe" . $i ?>" name="<?php echo "txtCe" . $i ?>" value="<?php if ($reg_fad['pfd4_ce'] == '') {
																																																					echo "";
																																																					$str_prop = '';
																																																				} else {
																																																					echo $reg_fad['pfd4_ce'];
																																																					$str_prop = 'disabled';
																																																				} ?>" <?php echo $str_prop;
																																																						echo " " . $strProp2;
																																																						echo " " . $strProp3;
																																																						echo " " . $strProp4; ?> size="5" placeholder="Ce" <?php if ($fltVal >= $reg_fad['pfd4_ce'] and $reg_fad['pfd4_ce'] != '') { ?>style="background-color:#66FF66;" <?php } ?>></td>

						<td><input type="text" class="form-control" id="<?php echo "txtObs" . $i ?>" name="<?php echo "txtObs" . $i ?>" value="<?php if ($reg_fad['pfd4_observaciones'] == '') {
																																					echo "";
																																					$str_prop = '';
																																				} else {
																																					echo $reg_fad['pfd4_observaciones'];
																																					$str_prop = 'disabled';
																																				} ?>" <?php echo $str_prop;
																																						echo " " . $strProp2;
																																						echo " " . $strProp3;
		...
		<div class="row footerdivProcesos" style="margin-bottom: 10px;">
			...
			<div class="form-group col-md-7">
				<div class="alert alert-info hide" id="alerta-errorFase4cOpe" style="height: 40px;width: 270px;text-align: left;z-index: 10;font-size: 10px;margin-bottom: -10px">
			...
			<?php if ($_SESSION['privilegio'] == 4) {	?>
				...
				<div class="col-md-1">
					<button type="button" class="btn btn-success" id="editar" onClick="javascript:AbreModalE4c(<?php echo $reg_pro['pro_id'] ?>, 9);">
	...
	<div class="modal" id="modalEditar4c" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false"></div>
