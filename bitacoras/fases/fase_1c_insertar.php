...
if ($txtFeIni != '') {
	...
	mysqli_query($cnx, "INSERT INTO procesos_fase_1_g(pro_id, pfg1_ph_agua,pfg1_ce_agua,pfg1_temp_ag, pe_id,pfg1_extractivilidad,pfg1_peroxido,pfg1_redox) VALUES('$hdd_pro_id', '$txtAgIni','$txtCeIni','$txtTemp', '$hdd_pe_id','0','$txt_peroxido',$txt_redox) ") or die(mysqli_error($cnx) . " Error al insertar 2");

	$respuesta = array('mensaje' => "Fase 1c agregada");
} else {
	for ($i = 1; $i <= 10; $i++) {
		$txtRen = ${"txtRen" . $i};
		$hddRen = ${"hddRen" . $i};
	...
	if ($strMsj == '') {
		if ($txtFeTerm != '' && $txtHrTerm != '' && $txt_redox != 'NULL') {
			...
			mysqli_query($cnx, "UPDATE procesos_fase_1_g SET pfg1_redox = $txt_redox WHERE pro_id = '$hdd_pro_id' and pe_id = '$hdd_pe_id' ") or die(mysqli_error($cnx) . " Error al actualizar 1");

			$strMsj = "Fase 1c actualizada";
		} else {
			if ($txtHrTotales != '') {
				...
				$strMsj = "Fase 1c parametros capturados";
			} else {
				$strMsj = "Esta vacio";
			}
		}
