...
/* $cad_inv2 = mysqli_query($cnx, "SELECT i.inv_id,i.inv_id_key, i.inv_no_ticket, i.inv_kg_totales,p.prv_nombre, i.prv_id, i.inv_fecha, i.inv_fe_recibe, i.inv_enviado, p.prv_tipo
   ...
   */
$cad_inv2 = mysqli_query($cnx, "SELECT i.inv_id,i.inv_id_key, i.inv_no_ticket, i.inv_kg_totales,p.prv_nombre, i.prv_id, i.inv_fecha, i.inv_fe_recibe, i.inv_enviado, p.prv_tipo, i.inv_folio_interno
	from inventario as i 
	inner join proveedores as p on (i.prv_id = p.prv_id)
	WHERE inv_id = " . $reg_inv['inv_id'] . "");
...
while ($row =  mysqli_fetch_array($cad_inv2)) {
 ...
 /*  echo '<option value="' . mb_convert_encoding($row['inv_id'], "UTF-8") . '"' . $var . '>';
  ...
  echo '</option>'; */
  $html= "<option style='text-align:right' value='".$row['inv_id']."'>".$row['inv_kg_totales']." / ".$row['prv_nombre']." / ".$row['inv_fecha']." / ".$row['inv_fe_recibe']. " (" . $row['inv_folio_interno'] . ")</option>";
