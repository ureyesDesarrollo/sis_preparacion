...
$cad_pro = mysqli_query($cnx, "SELECT p.*, e.ep_id
FROM procesos as p
...
<form id="formEncabezado2">
	...
	<input name="hdd_equipo" type="hidden" value="<?php echo $id_e ?>" id="hdd_equipo" />
	...
	<div class="form-row">
		...
		<div class="form-group col-md-3">
			...
			<select id="cbxLavador" class="form-control" style="width: 250px" name="cbxLavador" required disabled="disabled">
				...
				<?php
				$cadena =  mysqli_query($cnx, "SELECT * from equipos_preparacion");
				$registros =  mysqli_fetch_array($cadena);

				do {
				?><option value="<?php echo $registros['ep_id'] ?>" <?php if ($registros['ep_id'] == $reg_pro['ep_id']) { ?> selected="selected" <?php } ?>><?php echo $registros['ep_descripcion'] ?></option><?php
																																																			} while ($registros =  mysqli_fetch_array($cadena));
	...
	<div class="form-row">
		<div class="form-group col-md-2">
            ...
            WHERE pm.pro_id = '$reg_pro[pro_id]'") or die(mysqli_error($cnx) . "Error: en consultar el tipo de material");
