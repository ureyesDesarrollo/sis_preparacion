...
mysqli_query($cnx, "INSERT INTO procesos(pt_id, pro_total_kg, pro_supervisor) 
	VALUES('$cbxProceso', '$txtTotKilos', '$hdd_user')") or die(mysqli_error($cnx) . " Error al insertar procesos");
...
mysqli_query($cnx, "INSERT INTO procesos_equipos(pro_id, ep_id) 
	VALUES('$pro_id', '$cbx_equipo')") or die(mysqli_error($cnx) . " Error al insertar equipos");

$cad_p_j = mysqli_query($cnx, "SELECT MAX(pa_id) as clave FROM procesos_agrupados");
$reg_p_j = mysqli_fetch_array($cad_p_j);

$reg_p_j['clave'] += 1;

mysqli_query($cnx, "INSERT INTO procesos_agrupados(pa_id, pro_id) VALUES('$reg_p_j[clave]', '$pro_id')") or die(mysqli_error($cnx) . " Error al insertar agrupado");

mysqli_query($cnx, "UPDATE equipos_preparacion SET le_id = '10' WHERE ep_id = '$cbx_equipo' ") or die(mysqli_error($cnx) . " Error al actualizar"); //Ocupa el Lavador

//$cbxMaterial = 0;
//$cbxKilosID = 0;
for ($i = 1; $i <= 12; $i++) {

	if (isset(${"cbxKilosID" . $i})) {
	...
	if ($cbxKilosID != '') {
		...
		//Inserta el tipo_traje_detalle
		if (isset($reg_kg2['inv_fe_recibe'])) {
		...
		/* if ($cbxMaterial != '') { */
		/* if (isset($cbxMaterial) && isset($cbxKilosID)) { */
		if ($cbxMaterial != '') {
