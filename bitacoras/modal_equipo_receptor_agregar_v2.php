...
if ($cnx) {
	...
	//selecciona el dato agrupador del proceso principal
	...
	VALUES('$txtPro', '$cbxEquipo')") or die(mysqli_error($cnx) . " Error al insertar equipos");
		...
		do {
			...
			mysqli_query($cnx, "INSERT INTO bitacora_equipos (pro_id, pa_id, ep_anterior, ep_nuevo, usu_id, be_fecha,be_comentarios) VALUES ('$reg_pa_p[pro_id]', '$reg_pa_p[pa_id]', '$txtEquipo','$cbxEquipo', '" . $_SESSION['idUsu'] . "', '" . date("Y-m-d H:i:s") . "', 'Movimiento a equipo almacen vacio') ") or die(mysqli_error($cnx) . " Error al actualizar");

			//NUEVO - 29-11-2023 CC actualiza tiempo de proceso terminado
			mysqli_query($cnx, "UPDATE procesos SET  pro_estatus = '2', pro_fe_termino = '" . date("Y-m-d H:i:s") . "' where pro_id = '$reg_pa_p[pro_id]' ") or die(mysqli_error($cnx) . " Error al actualizar fecha termino proceso");
			...
			$horas_calculadas = fnc_horas($reg_procesos['pro_fe_carga'], $reg_procesos['fecha_termino'], $reg_procesos['pro_hr_inicio'], $reg_procesos['hora_termino']);
		...
		do {
			mysqli_query($cnx, "INSERT INTO bitacora_equipos (pro_id, pa_id, ep_anterior, ep_nuevo, usu_id, be_fecha,be_comentarios) VALUES ('$reg_x[pro_id]', '$reg_pa[pa_id]', '$txtEquipo','$cbxEquipo', '" . $_SESSION['idUsu'] . "', '" . date("Y-m-d H:i:s") . "', 'Movimiento a equipo almacen ocupado') ") or die(mysqli_error($cnx) . " Error al actualizar");

			//NUEVO - 29-11-2023 CC actualiza tiempo de proceso terminado
			mysqli_query($cnx, "UPDATE procesos SET  pro_estatus = '2', pro_fe_termino = '" . date("Y-m-d H:i:s") . "' where pro_id = '$reg_x[pro_id]' ") or die(mysqli_error($cnx) . " Error al actualizar fecha termino proceso");
