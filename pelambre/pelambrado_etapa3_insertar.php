...
try {
    ...
    for ($i = 4; $i <= 11; $i++) {
        ...
        $ipe_lote = valor_o_null(${"txt_lote" . $i});


        if ($hdd_id == '') {
            $query = "INSERT INTO inventario_pelambre_etapas_1 (
                ipe_ren, ip_id, ipe_etapa, ipe_porcentaje,ipe_cantidad, quim_id,ipe_lote, 
                ipe_horas, ipe_minutos, ipe_fe_hr_inicio, ipe_fe_hr_fin, 
                ipe_observaciones, usu_id
            ) VALUES (
                $renglon, $ip_id, 2,$ipe_porcentaje, $ipe_cantidad, $quim_id,$ipe_lote, 
                $ipe_horas, $ipe_minutos, $ipe_fe_hr_inicio, $ipe_fe_hr_fin, 
                $ipe_observaciones, $usu_id
            )";
        }
        if ($hdd_id != '') {
            $query = "UPDATE inventario_pelambre_etapas_1 SET ipe_lote = $ipe_lote, ipe_fe_hr_inicio = $ipe_fe_hr_inicio,ipe_fe_hr_fin = $ipe_fe_hr_fin, ipe_observaciones = $ipe_observaciones  WHERE ipe_id = '$hdd_id'";
        }
    ...
    $txt_fe_ter_encalado = validar_fecha("$txt_fe_ter_encalado");
    $txt_ph = valor_o_null("$txt_ph");
    $txt_lav = valor_o_null("$txt_lav");

    $query2 = "UPDATE inventario_pelambre SET ip_fe_hr_ter_encalado	 = $txt_fe_ter_encalado,ip_ph_encalado = $txt_ph,ip_lavado_encalado = $txt_lav WHERE ip_id = $ip_id";
