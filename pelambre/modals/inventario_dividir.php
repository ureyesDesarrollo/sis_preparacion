...
if ($txtSobra != '0') {
    ...
    mysqli_query($cnx, "INSERT INTO inventario(inv_fecha, inv_hora, inv_dia, inv_no_ticket, inv_placas, inv_camioneta, prv_id, mat_id, inv_kilos, inv_prueba, inv_desc_ag, inv_desc_d, inv_desc_ren, inv_kg_totales, inv_calidad, inv_no_factura, inv_peso_factura, inv_por_merma, inv_no_tarimas, inv_no_sacos,inv_enviado,inv_fe_enviado,inv_id_key,prv_recibe,inv_folio_interno,inv_hora_entrada,inv_hora_salida,ac_id,inv_solicitado) 
					...
					'$reg_inv[inv_no_tarimas]', '$reg_inv[inv_no_sacos]','5',
					'" . date("Y-m-d h:i:s") . "','$hdd_id','0',
					'$reg_inv[inv_folio_interno]',$hora_entrada,$hora_salida,'$reg_inv[ac_id]','')") or die(mysqli_error($cnx) . " Error al insertar");

    $reg_ultimo_id = mysqli_fetch_array(mysqli_query($cnx, "select MAX(inv_id) as res from inventario"));

    //Inserta inventario_pelambreF
    /*    mysqli_query($cnx, "INSERT INTO inventario_pelambre (inv_id,ep_id,fecha_envio,fecha_inicio,fecha_fin,usu_id) 
    VALUES(" . $reg_ultimo_id['res'] . ",'$slc_equipo','" . date("Y-m-d H:i:s") . "',NULL,NULL,'" . $_SESSION['idUsu'] . "')") or die(mysqli_error($cnx) . " Error al insertar");
 */
    ins_bit_acciones($_SESSION['idUsu'], 'A', $reg_ultimo_id['res'], '3');
} else {
    //actualiza a enviado
    mysqli_query($cnx, "UPDATE inventario SET inv_enviado = '5',prv_recibe = '0', inv_fe_enviado = '" . date("Y-m-d h:i:s") . "' WHERE inv_id = '$hdd_id' ") or die(mysqli_error($cnx) . " Error al actualizar el inventario");

    //Inserta inventario_pelambreF
    /*   mysqli_query($cnx, "INSERT INTO inventario_pelambre (inv_id,ep_id,fecha_envio,fecha_inicio,fecha_fin,usu_id) 
     VALUES('$hdd_id','$slc_equipo','" . date("Y-m-d H:i:s") . "',NULL,NULL,'" . $_SESSION['idUsu'] . "')") or die(mysqli_error($cnx) . " Error al insertar"); */
}

/* actualizar estatus */
/* mysqli_query($cnx, "UPDATE equipos_preparacion SET le_id  = '11' WHERE ep_id = '$slc_equipo' ") or die(mysqli_error($cnx) . " Error al actualizar estatus");
 */
