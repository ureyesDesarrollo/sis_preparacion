        ...
        function drenar(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '../indicadores/cambiar_estatus.php',
                        data: 'id=' + id,
                        type: 'post',
...
<body>
    <div class="container-fluid">
        ...
        <div class="row" style="box-shadow: 1px 2px 3px #e6e6e6">
            ...
            <div class="col-xs-12 col-sm-12 col-md-6" style="color: #000;text-align:right;margin-top:1.5rem">
                ...
                <!--  <?php if (
                            $_SESSION['privilegio'] == 1 || $_SESSION['privilegio'] == 2 || $_SESSION['privilegio'] == 4
                            || $_SESSION['privilegio'] == 7 || $_SESSION['privilegio'] == 15 || $_SESSION['privilegio'] == 5
                        ) { ?>
                    <a href="../modulos/mp_croquis.php" target="_blank">
                        <i class="fa-solid fa-splotch" aria-hidden="true"></i>
                        Patio MP
                    </a>
                <?php } ?> -->
                ...
                $parteDeseada = substr($urlCompleta, $pos + strlen("/sis_preparacion/"));
                ?>
                <a href=<?php echo "../seguridad/salir.php?url_tab=" . $parteDeseada ?>>
                    <i class="fa-solid fa-user"></i> Cerrar sesión
                </a>
            </div>
            <div class="row">
                <?php if (fnc_permiso($_SESSION['privilegio'], 3, 'upe_listar') == 1) { ?>
                    <div class="col-2"><a href="#listadoapelambre" onclick="enviar_a_pelambre()"><img src="../iconos/disp.png" />Enviar a pelambre</a></div>
                <?php } ?>
                <?php if (fnc_permiso($_SESSION['privilegio'], 3, 'upe_listar') == 1) { ?>
                    <div class="col-2"><a href="#listadoenpelambre" onclick="recibir_de_pelambre()"><img src="../iconos/disp.png" />Recibir pelambre</a></div>
                <?php } ?>
                <?php if (fnc_permiso($_SESSION['privilegio'], 3, 'upe_listar') == 1) { ?>
                    <div class="col-2">
                        <a href="bitacora/bitacora_pelambre.php" target="_blank">
                            <img class="icon" src="../iconos/bitacora2.png" alt="" width="20" height="20">
        ...
        ?>
                <div class="row">
                    <?php do {
                        $inv_pel = mysqli_query($cnx, "SELECT *, p.* FROM inventario_pelambre as ip
                                ...
                                <div class="card">
                                    <div class="card-body p-2 border fw-bold" style="background: <?php echo $reg_equipos['le_color'] ?>">
                                        <?php if ($reg_equipos['le_id'] == 10 or $reg_equipos['le_id'] == 11) { ?>
                                            <a href="pelambrado_bitacoras.php?id_e=<?php echo $reg_equipos['ep_id'] ?>" target="_blank" class="numero"><?php echo $reg_equipos['ep_descripcion']  ?></a>
                                        <?php } else {
                                    ...
                                    <div class="card-body p-1 border" style="font-size: .7rem; text-align: center;height:auto; height: 130px ">
                                        <?php do { ?>
                                            ...
                                            <?php echo "<span style='font-weight: bold;text-align:center'>Proveedor: </span>" . (($reg_autorizado['up_ban'] == 1) ? $reg_pel['prv_nombre'] : $reg_pel['prv_ncorto']); ?><br>
                                            <div class="p-3">
    ...
    <!-- Modal enviar a pelambre -->
    <div class="modal fade" id="modal_enviar_pelambre" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    </div>

    <!-- Modal recibir de pelambre -->
    <div class="modal fade" id="modal_recibir_pelambre" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    </div>
...
<script>
    ...
    function enviar_a_pelambre() {

        $.ajax({
            type: 'POST',
            url: 'modals/modal_enviar_pelambre.php',
            success: function(result) {
    ...
    function recibir_de_pelambre() {
        $.ajax({
            type: 'POST',
            url: 'modals/modal_recibir_pelambre.php',
            success: function(result) {
